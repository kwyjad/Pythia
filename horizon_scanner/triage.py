...
def _triage_model_spec(pass_idx: int = 1) -> ModelSpec:
    """Resolve the primary triage ModelSpec for a pass (env > config role).
    ...
    if pass_idx == 2:
        model_spec_str = os.getenv("PYTHIA_TRIAGE_MODEL_PASS2") or get_role_model("hs_triage_pass2")
    else:
        model_spec_str = os.getenv("PYTHIA_TRIAGE_MODEL_PASS1") or get_role_model("hs_triage_pass1")
...
async def _call_triage_model(
    ...
    *,
    ...
    Pass 1: Primary triage model (config role ``hs_triage_pass1``)
    Pass 2: Secondary triage model for diversity (config role ``hs_triage_pass2``)

    Override via env vars:
      PYTHIA_TRIAGE_MODEL_PASS1=google:gemini-3.5-flash
      PYTHIA_TRIAGE_MODEL_PASS2=google:gemini-3.5-flash

    Returns (text, usage, error, model_spec).
    """

    spec = _triage_model_spec(pass_idx)
    start = time.time()
    try:
...
def _run_triage_for_single_hazard(
    ...
    *,
    ...
    triage_mode = hs_batch.family_mode("hs_triage")
    if triage_mode == "submit":
        # Staged pipeline hs_rc_collect: enqueue the pass calls as Batch-API
        # requests and stop — parsing/merging happens in hs_finalize. A
        # non-batchable provider is simply not enqueued; its replay miss
        ...
        empty_pass = _build_single_triage_pass({})
        pending = _merge_single_triage_passes(empty_pass, empty_pass)
        pending["status"] = "pending_batch"
        pending["data_quality"]["status"] = "pending_batch"
        pending["valid"] = False
        return pending
    ...
    for pass_idx in range(1, _triage_pass_count() + 1):
        ...
        replayed = None
        if triage_mode == "collect":
            replayed = hs_batch.replay_hazard_call(
                "hs_triage", iso3=iso3, hazard_code=hazard_code, pass_idx=pass_idx
            )
            if replayed is None:
        ...
        _triage_label = f"triage_{hazard_code}_pass_{pass_idx}"
        if replayed is None or not hs_batch.llm_call_already_logged(run_id, iso3, _triage_label):
            log_hs_llm_call(
                hs_run_id=run_id,
                iso3=iso3,
                hazard_code=_triage_label,
...
def run_triage_for_country(
    ...
    *rc_results* is the output from :func:`run_rc_for_country` — its per-hazard
    ...
    # ran (and was persisted) during hs_rc_collect — reload the packs from
    # the DB instead of re-fetching (and re-paying for) searches.
    from horizon_scanner import hs_batch

    _triage_collect = hs_batch.family_mode("hs_triage") == "collect"

    if hazards_to_ground and _triage_collect:
        for hz in hazards_to_ground:
            grounding_results[hz] = hs_batch.load_grounding_pack(
                run_id, iso3_up, hz, prefer_prefix="triage_grounding"
            )
    ...
    # Persist triage grounding packs so the forecaster can inject them into
    # SPD prompts (skipped on collect — already persisted by hs_rc_collect).
    if not _triage_collect:
