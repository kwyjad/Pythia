...
Three phrasings have confused readers before. Do not use any of them.
...
- Do not add a unit to a placeholder that already carries one. Every figure
  resolves to a complete phrase, so "{{fig:excess_nominal}}" is written as it
  stands, never as "{{fig:excess_nominal}} more people". Doing so produced
  "nine and a half million people more people in crisis-level hunger" in a
  published report, and "five people more deaths".

Do not describe the shape of the distribution. The report prints the two
figures a planner acts on, and the full table of bands in the appendix. A
paragraph restating them in words sat between the reader and the decision.

## Do not write the sentence that fits any crisis

A phrase that could have been written before you read the pack is worth
nothing. Access constraints, funding gaps, compounding vulnerabilities,
protection risks: each of these is a real thing, and each is worth writing
only where the pack actually says it about this country this month. Where the
pack is silent, say less. The report is counted for these phrases and the
count is stored beside it.

Do not describe the machinery either. A reader does not want to hear about
base rates, distributions, ensembles or tails. They want to know what is
expected to happen and what to do about it.

## Length

Each entry is at most half a printed page. That is roughly one hundred and
eighty words across all its fields, and the analytical fields are the ones
worth the space. Say the important thing first. If you cannot fit a point,
drop it. The whole report body should read in ten minutes.
