...
Covers:
- Likelihood-driven level thresholds: L0 (<0.15), L1 (0.15-0.35),
  L2 (0.35-0.55), L3 (>=0.55)
...
# ---- Level thresholds (likelihood-driven) ----


class TestComputeLevel:
    """Verify likelihood-driven thresholds: L0 (<0.15), L1 (0.15-0.35),
    L2 (0.35-0.55), L3 (>=0.55)."""

    def test_level_0_low_likelihood(self):
        # likelihood < 0.15 => L0
        assert compute_level(0.10, 0.80, 0.08) == 0

    def test_level_0_zero(self):
        assert compute_level(0.0, 0.50, 0.0) == 0

    def test_level_0_borderline_below(self):
        # likelihood = 0.14 (just below 0.15) => L0
        assert compute_level(0.14, 0.90, 0.126) == 0

    def test_level_1_at_threshold(self):
        # likelihood = 0.15 => L1
        assert compute_level(0.15, 0.50, 0.075) == 1

    def test_level_1_mid_range(self):
        # likelihood = 0.25 => L1
        assert compute_level(0.25, 0.60, 0.15) == 1

    def test_level_1_syr_ace_case(self):
        # SYR ACE: likelihood=0.34 should now get L1 (was L0 before)
        assert compute_level(0.34, 0.335, 0.114) == 1

    def test_level_1_borderline_below_l2(self):
        # likelihood = 0.34 (just below 0.35) => L1
        assert compute_level(0.34, 0.80, 0.272) == 1

    def test_level_2_at_threshold(self):
        # likelihood = 0.35 => L2
        assert compute_level(0.35, 0.60, 0.21) == 2

    def test_level_2_mid_range(self):
        # likelihood = 0.50 => L2
        assert compute_level(0.50, 0.60, 0.30) == 2

    def test_level_2_borderline_below_l3(self):
        # likelihood = 0.54 (just below 0.55) => L2
        assert compute_level(0.54, 0.70, 0.378) == 2

    def test_level_3_at_threshold(self):
        # likelihood >= 0.55 => L3
        assert compute_level(0.55, 0.50, 0.275) == 3
    ...
    def test_none_likelihood(self):
        # None likelihood => L0
        assert compute_level(None, 0.80, 0.0) == 0

    def test_none_all(self):
        assert compute_level(None, None, None) == 0

    def test_env_override(self, monkeypatch):
        """Verify env vars still override thresholds."""
        monkeypatch.setenv("PYTHIA_HS_RC_LEVEL1_LIKELIHOOD", "0.10")
        monkeypatch.setenv("PYTHIA_HS_RC_LEVEL2_LIKELIHOOD", "0.30")
        monkeypatch.setenv("PYTHIA_HS_RC_LEVEL3_LIKELIHOOD", "0.50")
        # With lowered L1 threshold, likelihood=0.12 should be L1
        assert compute_level(0.12, 0.50, 0.06) == 1
        # With lowered L3 threshold, likelihood=0.52 should be L3
        assert compute_level(0.52, 0.50, 0.26) == 3
