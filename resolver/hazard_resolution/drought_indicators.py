...
#: Repository root, so a relative feed path means one file in every job.
...
#: A candidate with no scheme, or a ``file://`` one, is a path on disk.
#: The SPEI-3 country means are computed once by hand and COMMITTED
#: (``scripts/build_spei3_country_means.py``): turning a raster into a
#: country number is zonal statistics, which needs a scientific stack the
#: pipeline does not install and credentials it does not hold, so the file
#: is the product and the run just reads it.
_REMOTE_SCHEMES = ("http://", "https://", HDX_CKAN_SCHEME)


def _local_path(url: str) -> Path | None:
    """The path ``url`` names, or None when it names a remote address.

    A relative path is resolved against the repository root rather than
    the working directory: the pipeline runs these from several
    directories and a path that means different files in different jobs is
    worse than no path at all.
    """

    candidate = url.strip()
    if not candidate or candidate.startswith(_REMOTE_SCHEMES):
        return None
...
def _read_local(url: str) -> tuple[bytes, str]:
    """Read a committed feed off disk, or raise saying which path failed.

    Named loudly on a miss: a feed pointed at a file nobody generated is
    the hand-maintained-list failure, and the repair is to run the script
    that writes it, which only the path can tell you.
    """

    path = _local_path(url)
    if path is None:  # pragma: no cover - guarded by the caller
        raise ValueError(f"{url!r} is not a local path")
    if not path.exists():
        raise FileNotFoundError(
            f"{path} does not exist — this feed is generated by hand and "
            "committed; run scripts/build_spei3_country_means.py (or point "
            "the entry's url elsewhere) rather than waiting for it to appear"
        )
...
def _resolve_candidate(url: str, getter: GetFn, timeout: float) -> str:
    ...
    if not url.startswith(HDX_CKAN_SCHEME):
        return url
    # (a local path never reaches here: it does not carry the scheme)
