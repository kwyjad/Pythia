...
|---|---|---|---|---|
| ACLED | [`ingestion/config/acled.yml`](../ingestion/config/acled.yml) | Yes | `ACLED_REFRESH_TOKEN` (or `ACLED_USERNAME`/`ACLED_PASSWORD`) | OAuth client exchanges tokens via `https://acleddata.com/oauth/token`, caches the bearer credential, and pages 5 k rows from `https://acleddata.com/api/acled/read` with automatic retries before exposing the `monthly_fatalities` helper that groups totals by ISO3 + month for CLI previews; the paired `python -m resolver.cli.acled_to_duckdb` writer upserts into DuckDB table `acled_monthly_fatalities` keyed by `(iso3, month)` — see the [data dictionary entry](data_dictionary/acled_monthly_fatalities.md) for column details. ACLED outputs are *flows*: each country–month fatality total is independent and lands in `facts_deltas` with `series_semantics="new"`; `facts_resolved` intentionally remains empty for this connector.【F:resolver/ingestion/config/acled.yml†L1-L33】【F:resolver/ingestion/acled_auth.py†L1-L167】【F:resolver/ingestion/acled_client.py†L1223-L1390】【F:resolver/cli/acled_to_duckdb.py†L1-L205】 |
