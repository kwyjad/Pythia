...
- Country selection is always automatic. The connector calls the official SDK’s `get_all_countries()` catalog (falling back to `get_all_operations()` only when the primary call raises). Discovery snapshots are persisted to `diagnostics/ingestion/dtm/discovery_countries.csv` together with the SDK version and base URL. If discovery yields an empty set the run exits immediately with `discovery_fail.json` describing the failure. Any `api.countries:` list in config is ignored (but echoed in diagnostics for traceability).
...
- Diagnostics now capture dependency versions (`deps`), the effective parameters used (including `country_mode`, discovered country count, and value aliases), per-country row counts, and any per-country failures. Discovery metadata and file paths are recorded in the CSV meta manifest under `discovery`/`diagnostics`. Each fetch appends to `diagnostics/ingestion/dtm/requests.jsonl` with the level, selector, row count, elapsed time, and status, while `diagnostics/ingestion/dtm/dtm_http.ndjson` records the low-level HTTP traces (endpoint, status, elapsed ms, and error context). The first non-empty page per admin level is written to both `diagnostics/ingestion/raw/dtm/<level>.<country>.head.csv` and `diagnostics/ingestion/dtm/sample_<level>.csv` for schema inspection. If discovery or fetching fails, `diagnostics/ingestion/dtm/discovery_fail.json` documents the `reason` (`missing_key`, `invalid_key`, or `empty_discovery`) plus the SDK version, base URL, timestamp, and remediation hints.
