...
| Prediction Markets (optional) | Retrieves crowd forecasts from Metaculus, Polymarket, and Manifold Markets that are thematically related to the question. Uses LLM-generated search queries and relevance filtering to find the most informative market signals. These are presented as contextual evidence, not authoritative data. | Question metadata (country, hazard, time horizon). | Prediction market signals embedded in research briefs. |
| Research (question-level synthesis) | Produces a sourced research brief per question; audits HS Regime Change flags and assembles the reasoning substrate for forecasting. Optionally includes prediction market crowd forecasts as contextual signals. | HS triage + evidence pack + optional tail pack + optional prediction market signals + question definition. | question_research; merged evidence embedded in prompts. |
...
In addition to seasonal filtering, the system now ingests structured seasonal climate forecasts from the NMME (North American Multi-Model Ensemble) via the NOAA Climate Prediction Center. These forecasts provide country-level temperature and precipitation anomalies for the next seven months, expressed as standard deviations from climatology. For climate-sensitive hazards — drought, flood, heatwave, and tropical cyclone — these signals are injected into both HS triage and RC prompts as structured climate data, replacing what was previously an empty placeholder. The same seasonal outlook is also included in research and SPD forecaster prompts, giving the LLM ensemble concrete quantitative signals about expected climate conditions rather than relying solely on web-search-derived qualitative reports. The data is refreshed monthly via a scheduled pipeline that downloads, processes, and stores the forecasts in DuckDB.

For armed conflict (ACE) hazards, the system also ingests external conflict forecasts from two independent quantitative sources: VIEWS (Uppsala/PRIO), which provides ML-based predictions of fatalities and conflict onset probabilities at 1–6 month lead times, and conflictforecast.org (Mueller/Rauh), which provides news-based armed conflict risk scores at 3-month and 12-month horizons. These are stored in the database and injected into HS triage, RC, and forecaster prompts as structured quantitative anchors alongside the ACLED retrospective base rates. Where the two sources disagree, the LLM is instructed to note the disagreement and reason about why — VIEWS is stronger at capturing trends and baseline levels, while conflictforecast.org is better at detecting shifts from media coverage patterns.

In addition, ICG CrisisWatch expert assessments are incorporated at prompt time via web research. The system fetches per-country CrisisWatch entries (directional arrows: Deteriorated/Improved/Unchanged) and the monthly "On the Horizon" feature, which highlights roughly three conflict risks and one resolution opportunity expected in the next three to six months. Countries flagged by ICG's "On the Horizon" receive a prominent note in their RC assessment, as ICG is highly selective about what it flags.
...
- **Conflict forecasts**: External quantitative predictions for armed conflict, sourced from VIEWS (ML-based fatality predictions) and conflictforecast.org (news-based risk scores). Stored in the `conflict_forecasts` table and injected into ACE prompts alongside ACLED base rates.
