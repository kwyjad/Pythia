...
- **Source-priority steering in grounding prompts.** All 10 grounding prompts (5 RC + 5 triage) now include a "PRIORITIZE THESE SOURCES" section with hazard-specific source priority lists and a recency filter instruction. RC prompts prioritize wire services and specialist sources for novelty detection; triage prompts elevate OCHA and humanitarian sources for the operational picture. Grounding queries also now use hazard-specific search labels (e.g., "armed conflict escalation signals" for ACE, "flood risk river levels" for FL) instead of generic terms.
- **ENSO and seasonal TC forecasts now in forecaster SPD prompts.** ENSO state and forecast data is now injected into SPD prompts for climate-sensitive hazards (DR, FL, HW, TC), and seasonal TC forecasts are injected for TC questions. These were previously available only to the Horizon Scanner.
...
| ENSO state and forecast (IRI/CPC) | DR, FL, HW, TC | DR, FL, HW, TC | DR, FL, HW, TC |
...
Recency windows for RC grounding are much shorter than for triage, reflecting RC's focus on very recent disruptions: ACE uses 14 days, DR uses 30 days, FL uses 14 days, HW uses 14 days, and TC uses 30 days. All RC grounding prompts include a source-priority list that favors wire services and specialist sources for novelty detection, and hazard-specific search labels (e.g., "armed conflict escalation signals" for ACE) to improve search relevance.
