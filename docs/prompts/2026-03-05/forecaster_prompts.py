...
def build_spd_prompt_v2(
    ...
    if hazard == "DI" and metric == "PA":
        di_instruction = (
            "- For DI (displacement inflow), there is no Resolver base rate. Construct your **base rate** from the structured evidence "
            "(IPC projections, ACAPS monitoring, ReliefWeb reports): look at past monthly inflows, typical ranges, and relevant reference cases, then use that as your prior.\n"
            "- Use HS triage and update signals from structured evidence to adjust this base rate up or down, but do not fabricate a history where none exists.\n"
    ...
    return (
        ...
        f"{hazard_nat_note}"
        f"{di_instruction}"
        "- Use the structured evidence and HS triage to identify update signals that push risk up or down.\n"
        "- Avoid large deviations from the base rate unless the evidence strongly supports them.\n"
        f"{base_rate_note}"
        "- If Resolver history is missing (`source` = \"none\"), rely on HS triage + structured evidence to shape the base rate.\n"
