...
def build_rc_prompt_ace(
    ...
    icg_on_the_horizon: Optional[str] = None,  # deprecated — use crisiswatch_context
    ...
    **kwargs: Any,
    ...
    # Build optional ICG CrisisWatch section (prefer new crisiswatch_context)
    icg_text = crisiswatch_context or icg_on_the_horizon
    icg_section = ""
    if icg_text:
        icg_section = f"""
{icg_text}
...
12-month average, conflictforecast.org flags escalation, or ACLED CAST
predicts notably higher event counts), this supports a higher RC likelihood.
If forecasts are consistent with base rates, this supports stability.
"""
...
def build_rc_prompt_dr(
    ...
    **kwargs: Any,
    ...
    enso_context : optional ENSO state and forecast text block from IRI/CPC.
    """
    ...
    # Format additional data sources
    additional_context = _format_new_data_sections(
        reliefweb_reports=reliefweb_reports,
        ipc_phases=ipc_phases,
        fewsnet_food_security=fewsnet_food_security,
...
def build_rc_prompt_tc(
    ...
    **kwargs: Any,
    ...
    seasonal_tc_context : optional pre-scraped seasonal TC forecast text
        from TSR, NOAA CPC, and/or BoM (loaded from cached JSON).
    enso_context : optional ENSO state and forecast text block from IRI/CPC.
    """
