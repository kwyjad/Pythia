...
# Gemini models for Google Search grounding (used as fallback when Brave/
# OpenAI web search is primary). Preview models have shown intermittent
# regressions, so the role defaults to the cheap stable tier (gemini_lite).
# Override with PYTHIA_GROUNDING_MODEL_ID env var (inserted before these).


def _default_gemini_candidates() -> List[str]:
    from pythia.llm_profiles import get_role_model, split_model_ref

    return [split_model_ref(get_role_model("grounding_gemini"))[1]]


DEFAULT_GEMINI_CANDIDATES = _default_gemini_candidates()
MAX_SIGNAL_LINES = 8
...
def fetch_unverified_summary_via_gemini(
    query: str,
    *,
    recency_days: int,
    include_structural: bool,
    ...
    """Fetch an unverified summary via Gemini without enabling any tools."""
    ...
    if not api_key:
        pack.error = {"type": "missing_api_key", "message": "GEMINI_API_KEY / GOOGLE_API_KEY not set"}
        pack.debug = {"error": "missing_api_key", "unverified_summary_used": True}
        return pack

    prompt = (
        "You are a research assistant. Provide an unverified summary without using any external tools. "
        "Return strictly JSON with this shape:\n"
        "{\n  \"structural_context\": \"max 8 lines\",\n"
        "  \"recent_signals\": [\"<=8 bullets, last 120 days\"],\n  \"notes\": \"optional\"\n}"
        "\n- Focus on authoritative, recent sources (last "
    ...
    for model_id in model_candidates:
        attempted_models.append(model_id)
        url = f"https://generativelanguage.googleapis.com/v1beta/models/{model_id}:generateContent"
        status_code, response_data = _post(url, body)
        if status_code == 200:
            selected_model_id = model_id
            break
        msg = ""
        if isinstance(response_data, dict):
            if "error" in response_data and isinstance(response_data["error"], dict):
                msg = response_data["error"].get("message", "") or ""
            if not msg:
                msg = response_data.get("error_text", "")
        if not msg:
    ...
    if usage["total_tokens"] == 0 and text_blob:
        approx_total = max(len(prompt.split()) + len(text_blob.split()), 1)
        usage["prompt_tokens"] = usage["prompt_tokens"] or max(len(prompt.split()), 1)
        usage["completion_tokens"] = usage["completion_tokens"] or max(len(text_blob.split()), 1)
        usage["total_tokens"] = approx_total
    ...
    pack.debug = {
        **extra_debug,
        ...
        "provider": provider_name,
        "fetched_at": datetime.utcnow().isoformat(),
        "unverified_summary_used": True,
    }
    if last_errors:
        pack.debug["last_errors"] = last_errors[:5]

    if status_code != 200:
        message = "; ".join(last_errors[:3]) if last_errors else f"status={status_code}"
        pack.error = {"type": "provider_error", "message": message}
    elif not text_blob:
        pack.error = {"type": "empty_response", "message": "no response text returned"}
