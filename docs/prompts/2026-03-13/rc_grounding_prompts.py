...
RECENCY_DAYS = {
    "ACE": 14,   # Conflict escalation signals should be very recent
    "DR": 30,    # Drought onset signals over the past month
    "FL": 14,    # Flood events are acute
    "HW": 14,    # Heatwave events are acute
    "TC": 30,    # TC season outlooks + active storm tracking
}
...
- Quality broadcasters: BBC (including BBC Arabic, BBC Persian, BBC Somali), Al Jazeera, France 24, Deutsche Welle
- Specialist conflict sources: International Crisis Group, ACLED analysis, The New Humanitarian, Armed Conflict Location & Event Data Project
- Regional outlets: AllAfrica, The Diplomat, Middle East Eye
- UN/humanitarian: OCHA ReliefWeb, UNHCR, ICRC
- Search for headlines from: The Economist, Bloomberg, Foreign Policy, Foreign Affairs (even if paywalled, headlines and lead paragraphs are often accessible and contain key signals)

RECENCY FILTER: Strongly prefer sources published within the last 14 days. Older sources are acceptable only for slow-moving structural context.
...
PRIORITIZE THESE SOURCES (in order of reliability):
- Food security specialists: FEWS NET, FAO GIEWS, WFP VAM, IPC
- Climate services: IRI, CPC, ECMWF seasonal outlooks, Copernicus Climate Change Service
- Wire services: Reuters, AP, AFP
- Humanitarian: OCHA ReliefWeb, The New Humanitarian
- Quality broadcasters: BBC, Al Jazeera, France 24, Deutsche Welle
- Regional: AllAfrica, The Diplomat

RECENCY FILTER: Prefer sources from the last 30 days. Seasonal forecast outlooks up to 60 days old are acceptable.
...
PRIORITIZE THESE SOURCES (in order of reliability):
- Flood monitoring: FloodList, GloFAS bulletins, national hydrological services
- Humanitarian: OCHA (flash updates, situation reports), IFRC, ReliefWeb
- Climate services: WMO, IRI, CPC, ECMWF
- Wire services: Reuters, AP, AFP
- Quality broadcasters: BBC, Al Jazeera, France 24, Deutsche Welle
- NASA Earth Observatory for satellite-based flood extent

RECENCY FILTER: Strongly prefer sources from the last 14 days. Active flood events change daily.
...
PRIORITIZE THESE SOURCES (in order of reliability):
- Climate services: WMO, national meteorological services, Copernicus C3S, IRI, CPC, ECMWF
- Health: WHO heat-health warnings, national health ministry bulletins
- Wire services: Reuters, AP, AFP
- Quality broadcasters: BBC, Al Jazeera, France 24, Deutsche Welle
- Energy/infrastructure: sector reporting on grid stress, water supply
- Humanitarian: OCHA, The New Humanitarian

RECENCY FILTER: Strongly prefer sources from the last 14 days. Seasonal temperature outlooks up to 60 days old are acceptable.
...
PRIORITIZE THESE SOURCES (in order of reliability):
- Official TC advisory centers: NOAA NHC, JTWC, regional RSMCs (JMA Tokyo, IMD, Météo-France La Réunion, BoM)
- Seasonal forecasts: TSR (UCL), Colorado State University, NOAA/CPC seasonal outlook
- Humanitarian: OCHA flash updates, IFRC emergency appeals
- Wire services: Reuters, AP, AFP
- Quality broadcasters: BBC, Al Jazeera, France 24
- Climate context: WMO, IRI (for ENSO)

RECENCY FILTER: For active storms, only the last 3-5 days matter. For seasonal outlooks, up to 60 days is acceptable.
