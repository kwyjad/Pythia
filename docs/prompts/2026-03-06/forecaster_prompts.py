...
def _load_calibration_advice_for_hazard(hazard_code: str, metric: str) -> str:
    """Load hazard-specific calibration advice from DB, falling back to global."""
    ...
    try:
        ...
        con = duckdb_io.get_db(db_url)
        try:
            # Try hazard-specific advice first
            row = con.execute(
                """
                SELECT advice
                FROM calibration_advice
...
def build_spd_prompt_v2(
    ...
    cal_advice_text = _load_calibration_advice_for_hazard(hazard, metric)
    calibration_section = ""
    if cal_advice_text:
        calibration_section = (
