...
- **Both RC and triage passes now use Gemini Flash.** RC and triage assessments previously used a two-model approach (Gemini Flash for Pass 1 and GPT-5-mini for Pass 2). Both RC passes and both triage passes now default to Gemini Flash (configurable via environment variables).
...
2. **Per-hazard RC assessment** — For each hazard, the RC LLM is invoked with the grounding results plus all relevant structured data for that hazard. The RC prompt is hazard-specific (see Section 7 for details). RC runs two passes, both using Gemini Flash by default (configurable via environment variables). The results are merged by averaging likelihood and magnitude and reconciling direction labels. RC calls run sequentially (not in parallel) to manage rate limits and cost.
...
4. **Per-hazard triage assessment** — For each hazard, the triage LLM is invoked with the triage grounding results, the RC result from step 2, and all relevant structured data for that hazard. The triage prompt is hazard-specific (see below). Triage also runs two passes (both defaulting to Gemini Flash, configurable via environment variables) with averaged results. The ACLED low-activity filter is applied before ACE triage: if a country has 0 fatalities in the 2 most recent months and fewer than 25 fatalities in the trailing 12 months, ACE triage is skipped entirely and a default quiet score is assigned.
...
To reduce run-to-run variance, HS uses a stabilization pattern: both RC and triage run a two-pass pipeline and average numeric outputs such as triage_score and RC likelihood. Both RC and triage passes currently default to Gemini Flash (configurable via environment variables). This is not a deep statistical ensemble; it is a practical technique that improves consistency for dashboard users.
...
Note that some data sources available to the Horizon Scanner are not passed through to the forecaster. ACAPS Humanitarian Access and ACLED monthly summaries are used during HS triage and RC assessment but are not directly injected into the SPD prompt. ICG CrisisWatch data is loaded by the forecaster but is not currently rendered into the SPD prompt text — its influence reaches the forecaster indirectly through the HS triage output and RC fields. ENSO state/forecast and seasonal TC forecasts are now injected into SPD prompts for their respective hazard types (see Section 4 summary table).
