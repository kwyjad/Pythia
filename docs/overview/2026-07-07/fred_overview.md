...
This version also reflects changes from PRs #643–#738, building on the per-hazard architecture introduced in PR #641. Key changes include Brave Search API as the primary grounding backend, tighter grounding recency windows, lowered thresholds for adversarial checks and tail packs, likelihood-only RC levels, a redesigned bulk-fetch structured data ingestion pipeline, official deprecation of the question-level web research pipeline, source-priority steering in grounding prompts, model-aware calibration advice, a new ICG CrisisWatch connector with Playwright-based scraping, binary event forecasts (EVENT_OCCURRENCE), FEWS NET IPC Phase 3+ integration, IPC API connector for non-FEWS NET countries, GDACS as a Resolver connector, structured reasoning traces, LLM transcript viewing, DuckDB memory management fixes, DB-first context data loading, a consolidated single-job backfill workflow, epoch-specific question IDs, CrisisWatch injection into SPD prompts, and source-aware null handling for resolutions.

- **Tighter grounding recency windows.** RC grounding recency windows have been substantially tightened to focus on recent disruptions: ACE uses 30 days, DR uses 60 days, FL uses 30 days, and TC uses 60 days. These were further widened from an initial aggressive tightening (14/30 days) to balance change-signal detection with sufficient search coverage for a monthly forecast cycle. Triage grounding windows were also tightened: ACE uses 60 days, DR uses 90 days, FL uses 60 days, and TC uses 60 days. The intent is that RC grounding hunts for recent change signals, while triage grounding captures a somewhat wider operational picture.
- **Adversarial checks and tail packs now trigger at RC Level 1+.** Previously, adversarial evidence checks and hazard tail packs were only generated for RC Level 2 or above. They now trigger at Level 1 or above, providing counter-evidence and trigger detection for a wider range of elevated-risk cases. Level 2+ still additionally forces a full SPD forecast to be produced. Adversarial checks are injected into the SPD forecasting prompt only for Level 2+ cases.
- **Likelihood-only RC level thresholds.** RC levels are now determined solely by the likelihood dimension: Level 0 (likelihood < 0.15), Level 1 (likelihood >= 0.15), Level 2 (likelihood >= 0.35), Level 3 (likelihood >= 0.55). The previous composite-score thresholds have been removed in favor of this simpler, likelihood-only scheme.
- **RC passes now both use Gemini Flash; triage uses Gemini Pro + Flash.** RC assessments run two passes, both defaulting to Gemini 3 Flash (configurable via `PYTHIA_RC_MODEL_PASS1` / `PYTHIA_RC_MODEL_PASS2`). Triage assessments also run two passes: Pass 1 defaults to Gemini 3.1 Pro (resolved from the ensemble config) and Pass 2 defaults to Gemini 3 Flash (configurable via `PYTHIA_TRIAGE_MODEL_PASS1` / `PYTHIA_TRIAGE_MODEL_PASS2`).
- **DI, CU, and HW hazards fully silenced.** Displacement Influx, Civil Unrest, and Heatwave hazards have been completely removed from the active pipeline. They are blocked at the hazard catalog level and never enter RC assessment, triage, grounding, or question generation. The pipeline now processes exactly four hazards per country: ACE, DR, FL, and TC (minus seasonal screen-outs for TC). This eliminates all wasted LLM and grounding calls on these unforecastable hazards.
- **Executive summary redesigned.** The debug bundle executive summary now splits the old "Horizon Scan" section into three explicit stages — RC Assessment, Triage, and Question Generation — each labeled with its unit of counting (hazard-country pairs vs. forecast questions). The Grounding section now reports "with sources" and "empty" counts instead of the misleading "fails" label. The Structured Data Injects section now shows which countries are missing each data source.
- **Source-priority steering in grounding prompts.** All 8 grounding prompts (4 RC + 4 triage) now include a "PRIORITIZE THESE SOURCES" section with hazard-specific source priority lists and a recency filter instruction. RC prompts prioritize wire services and specialist sources for novelty detection; triage prompts elevate OCHA and humanitarian sources for the operational picture. Grounding queries also now use hazard-specific search labels (e.g., "armed conflict violence displacement humanitarian situation" for ACE, "flooding displacement humanitarian impact" for FL) instead of generic terms.
- **Brave Search API as primary grounding backend.** Web search grounding now uses the Brave Search API as the primary backend, replacing both OpenAI web search and Gemini Google Search grounding. Brave provides deterministic direct web search with freshness filtering. A three-tier fallback chain operates: Brave → OpenAI → Gemini. The same chain is used for RC grounding, triage grounding, and adversarial evidence searches. A thread-safe rate limiter prevents exceeding the API's request-per-second limit during all-country runs.
- **IPC API connector for non-FEWS NET countries.** A new IPC API connector fetches Phase 3+ population data from api.ipcinfo.org for countries not monitored by FEWS NET (e.g., Afghanistan, Pakistan, Guatemala). This extends food security ground truth coverage beyond the 48 FEWS NET-monitored countries. The unified food security loader tries FEWS NET first and falls back to IPC API. DR questions can now be generated for IPC API countries as well.
- **ENSO and seasonal TC forecasts now in forecaster SPD prompts.** ENSO state and forecast data is now injected into SPD prompts for climate-sensitive hazards (DR, FL, TC), and seasonal TC forecasts are injected for TC questions. These were previously available only to the Horizon Scanner.
- **HDX Signals and conflict forecasts now in forecaster SPD prompts.** HDX Signals alerts and conflict forecast text (VIEWS, conflictforecast.org, ACLED CAST) are now loaded into the forecaster's structured data and injected into SPD prompts, in addition to their existing use in the Horizon Scanner.
- **Model-aware calibration advice.** Calibration advice is now generated per model (not just per hazard/metric), allowing the system to provide model-specific feedback — for example, noting that a particular model tends to under-weight tail risk for a given hazard.
- **Bulk-fetch structured data ingestion.** The structured data ingestion pipeline was redesigned from per-country sequential API calls to bulk global fetches with concurrent processing, reducing ingestion runtime from over 6 hours to approximately 5–10 minutes.
- **ICG CrisisWatch connector with Playwright scraper.** CrisisWatch data is now fetched from a locally cached JSON file produced by a Playwright-based scraper that runs monthly via a GitHub Actions workflow. The scraper loads the ICG CrisisWatch website in headless Chromium, parses per-country directional assessments and "On the Horizon" flags, and commits the structured output as JSON. A Gemini grounding fallback is only attempted when the JSON file is missing or empty. CrisisWatch is fetched once at the start of each Horizon Scanner run and passed through to all country workers. Data is persisted in a DuckDB table and injected into ACE RC and ACE triage prompts.
- **Question-level web research pipeline deprecated.** The former "Research LLM" stage that generated per-question web search briefs is now fully bypassed. The forecaster receives all its evidence through structured data connectors and Horizon Scanner outputs. The `question_research` table is no longer populated by the pipeline.
- **NMME ingestion merged into structured-data workflow.** The standalone NMME ingestion workflow has been deprecated. NMME seasonal forecast ingestion now runs as a step within the main structured-data ingestion workflow, with graceful handling for the fact that CPC FTP files are typically published around the 9th–10th of each month.
- **Structured reasoning traces.** The forecaster now captures structured reasoning from each model, recording the prior distribution, sequential evidence updates with numeric deltas, and the model's response to Regime Change flags. These traces enable transparency into why different models produce different forecasts, and feed into the calibration advice system for diagnosing systematic biases in prior anchoring.
- **Binary event forecasts (EVENT_OCCURRENCE).** The system now produces binary probability forecasts for flood, drought, and tropical cyclone event occurrence, complementing the existing SPD-based magnitude forecasts. Binary questions use GDACS alert data (Orange/Red = event) as ground truth and are scored using Brier score.
- **Separate accuracy scores for binary vs. magnitude forecasts.** The two kinds of forecast are graded on different scales — a binary yes/no Brier score ranges from 0 to 1 and is naturally very low when a rare event correctly doesn't happen, while a five-bucket magnitude (SPD) Brier score ranges from 0 to 2. Mixing them into one "average accuracy" number is misleading: it makes overall accuracy look far better than it is for the magnitude forecasts. The performance dashboard now reports these as two clearly separated figures ("SPD Brier" and "Binary Brier", each with its own question count), and the downloadable scores files include a `metric_family` column so anyone averaging the numbers groups the two families separately. This resolves a confusing case where a spreadsheet average of a downloaded scores file (0.20) didn't match the dashboard's headline accuracy (0.62) — they were measuring different sets of questions on different scales.
- **FEWS NET IPC Phase 3+ integration.** Drought questions in FEWS NET-monitored countries now use the PHASE3PLUS_IN_NEED metric with IPC Phase 3+ population data from the FEWS NET Data Warehouse, replacing the unavailable IPC API. This provides more precise food-security ground truth for forecast resolution and scoring.
- **GDACS as a Resolver connector.** GDACS disaster data is now integrated into the Resolver pipeline, providing both population exposure (`in_need`) and binary event occurrence (`event_occurrence`) metrics for FL, DR, and TC hazards. GDACS event history is injected into both SPD and binary forecast prompts as structured base rate context.
- **LLM transcript viewer and regime-change UI.** The question detail page now includes an LLM transcript viewer showing prompts and responses grouped by pipeline stage, and a regime-change summary panel with rationale, triggers, and confidence assessment.
- **DuckDB memory limits and concurrency control.** Fixed OOM crashes by capping DuckDB memory usage, eliminating pandas dependencies in critical paths, and adding concurrency controls for large query workloads.
- **Epoch-specific question IDs.** Question IDs now include an epoch suffix (e.g., `ETH_ACE_FATALITIES_2026-04`) to prevent overwrites when the HS runs again in a new month. The questions table is append-only: the original metadata (window start date, target month, wording) is preserved rather than overwritten. The question's own `window_start_date` is treated as authoritative for resolution.
- **DB-first context data loading.** Context sources (ENSO, seasonal TC, HDX Signals, CrisisWatch) now follow a DB-first pattern: data is fetched and stored in DB tables during the backfill workflow, and prompt loaders read from the DB first (falling back to live fetch or file cache if empty). This ensures the DB artifact is fully self-contained and reproducible.
- **Consolidated single-job backfill workflow.** The `resolver_update.yml` workflow was consolidated into a single-job workflow with 5 sequential phases (Resolver connectors, resolution sources, structured data, context sources, verify+export). Phase 1 failures are fatal; phases 2-4 use continue-on-error so individual source failures don't block the entire backfill.
- **CrisisWatch injected into SPD prompts.** ICG CrisisWatch data is now loaded by the forecaster's structured data loader and injected into SPD prompts for ACE hazards, in addition to its existing use in RC and triage prompts.
- **Triage Pass 1 now uses Gemini 3.1 Pro.** Triage Pass 1 defaults to Gemini 3.1 Pro (resolved from the ensemble config) for greater analytical depth, while Pass 2 uses Gemini 3 Flash for speed/cost balance. RC continues to use Gemini 3 Flash for both passes.
- **Source-aware null handling for resolutions.** Resolution now uses source-aware logic: FATALITIES and EVENT_OCCURRENCE default to 0 when no data exists (ACLED/GDACS have continuous coverage), while PA and PHASE3PLUS_IN_NEED leave horizons unresolved when no data exists (IFRC/FEWS NET coverage is incomplete). Unresolved horizons are excluded from scoring, preventing false-zero contamination.
- **Prediction market retriever disabled.** The prediction market retriever is now disabled by default due to upstream API failures (Metaculus 403, Polymarket 422).
...
| Web Research (evidence packs) | Fetches grounded, source-cited qualitative evidence to complement quantitative facts, with an explicit recency window. Used by the Horizon Scanner for grounding calls. The former question-level web research pipeline (which generated per-question research briefs for the forecaster) has been deprecated; the forecaster now receives all evidence through structured data connectors. | Country and hazard-tail queries; web research backend. | Grounding evidence used by HS triage and RC; sources lists. |
| Structured Data Connectors | Pulls authoritative humanitarian data and conflict/climate forecasts from specialist APIs and stores it for prompt injection. Replaces the former research LLM stage. | ReliefWeb, ACAPS (4 datasets), IPC, ACLED political events, VIEWS, ConflictForecast, NOAA, ACLED CAST, HDX Signals, GDELT, IRI/CPC ENSO, TSR/NOAA CPC/BoM seasonal TC. | Database tables with structured evidence; formatted prompt blocks for forecasting. |
...
| Hazard Tail Packs (conditional) | Runs hazard-specific follow-up evidence retrieval for high RC cases to find triggers and counter-signals. | HS RC flags (Level >= 1), hazard code, forecast window. | hs_hazard_tail_packs (cached per run/country/hazard). |
...
Fred now maintains fifteen structured data feeds, organized into five categories: humanitarian monitoring, food security, climate and seasonal forecasts, conflict forecasts, and automated crisis monitoring. These feeds are used across the entire pipeline — Horizon Scanner RC assessment, HS triage, and the forecaster — though not every feed appears in every prompt. The table at the end of this section summarizes which feeds are injected where.
...
**IPC API Phase 3+ Population Estimates.** The IPC API connector fetches Phase 3+ population data from api.ipcinfo.org for countries not covered by FEWS NET. It requires an `IPC_API_KEY` environment variable and explicitly excludes FEWS NET countries (FEWS NET always takes priority). This extends food security ground truth beyond the 48 FEWS NET-monitored countries to additional food-insecure countries. The unified food security loader (`food_security.py`) tries FEWS NET first and falls back to IPC API, so DR questions can now be generated and resolved for any country with food security data from either source. A legacy `ipc_phases` table exists in the codebase from an earlier connector but is no longer populated or used.
...
| Seasonal TC forecasts (TSR/NOAA/BoM) | TC | TC | TC |
...
2. **Per-hazard RC assessment** — For each hazard, the RC LLM is invoked with the grounding results plus all relevant structured data for that hazard. The RC prompt is hazard-specific (see Section 7 for details). RC runs two passes, both defaulting to Gemini 3 Flash (configurable via `PYTHIA_RC_MODEL_PASS1` / `PYTHIA_RC_MODEL_PASS2`). The results are merged by averaging likelihood and magnitude and reconciling direction labels. RC calls run sequentially (not in parallel) to manage rate limits and cost.
...
4. **Per-hazard triage assessment** — For each hazard, the triage LLM is invoked with the triage grounding results, the RC result from step 2, and all relevant structured data for that hazard. The triage prompt is hazard-specific (see below). Triage runs two passes: Pass 1 defaults to Gemini 3.1 Pro (resolved from the ensemble config) and Pass 2 defaults to Gemini 3 Flash (configurable via `PYTHIA_TRIAGE_MODEL_PASS1` / `PYTHIA_TRIAGE_MODEL_PASS2`). Results are merged by averaging. The ACLED low-activity filter is applied before ACE triage: if a country has 0 fatalities in the 2 most recent months and fewer than 25 fatalities in the trailing 12 months, ACE triage is skipped entirely and a default quiet score is assigned.
...
To reduce run-to-run variance, HS uses a stabilization pattern: both RC and triage run a two-pass pipeline and average numeric outputs such as triage_score and RC likelihood. RC uses Gemini 3 Flash for both passes; triage uses Gemini 3.1 Pro for Pass 1 and Gemini 3 Flash for Pass 2 (all configurable via environment variables). This is not a deep statistical ensemble; it is a practical technique that improves consistency for dashboard users.
...
The adversarial check is stored in the database. It is generated for RC Level 1+ cases, providing counter-evidence for all forecasts flagged as potentially deviating from base rates. The adversarial check is injected into the SPD forecasting prompt only for RC Level 2+ cases, ensuring the forecasting models consider both the evidence supporting a regime shift and the evidence against it for the most elevated-risk situations.
...
Tail packs are generated conditionally. The system generates tail packs for hazards with RC Level 1 or above, and it caps generation to at most two hazards per country per HS run (by RC score). This provides trigger evidence for a wider range of elevated-risk cases while keeping costs bounded. Tail packs are cached per run, country, and hazard in the database so reruns do not repeatedly hit the web.
...
**Note:** The prediction market retriever is currently disabled by default (`PYTHIA_PM_RETRIEVER_ENABLED=0`). Metaculus returns HTTP 403, Polymarket returns HTTP 422, and each call times out at 30 seconds, adding approximately 6 minutes of wasted wall time per run. The retriever should not be re-enabled until the upstream APIs are fixed.
...
The system includes a self-search capability where the model can request a single additional web query when evidence is insufficient. However, self-search is disabled by default along with the deprecated web research pipeline. It requires multiple environment variables to be enabled (`PYTHIA_FORECASTER_SELF_SEARCH`, `PYTHIA_SPD_WEB_SEARCH_ENABLED`, `PYTHIA_MODEL_SELF_SEARCH_ENABLED`) and is not used in production.
...
The Performance page surfaces forecast evaluation in the dashboard. It displays five KPI cards (scored questions, hazards, models, HS runs, and ensemble Brier score) and supports four views: Total (one row per model), By Hazard (scores broken down by hazard code), By Run (scores per HS run over time), and By Model. Users can filter by metric scope: People Affected (PA), Fatalities, Event Occurrence (binary), or Phase 3+ (IPC food security). A Resolution Coverage Summary section shows resolution rates by hazard and metric, color-coded by completeness (green >90%, yellow 50-90%, red <50%). An ensemble selector dropdown lets users switch between named ensemble methods (e.g., ensemble_mean vs. ensemble_bayesmc) in the By Hazard and By Run views, enabling direct comparison of aggregation strategies. Tables show both average and median values for all three scoring rules (Brier, Log, CRPS); median scores are particularly useful because they are less sensitive to outliers from early-run or data-sparse questions.
...
Scheduled workflows include: one for refreshing the ENSO state cache (scraping IRI/CPC), one for refreshing the seasonal TC forecast cache (running the TSR, NOAA CPC, and BoM scrapers), one for refreshing ICG CrisisWatch data (Playwright-based monthly scraper), the primary backfill workflow (15th of each month, running all data connectors in five sequential phases), and a mid-cycle structured data refresh (weekly on Sunday 03:00 UTC) that updates fast-changing sources — conflict forecasts, GDACS, ReliefWeb, and ACLED political events. The mid-cycle refresh ensures that these rapidly-evolving data sources stay current between monthly backfills, so forecasts made mid-month benefit from the latest conflict and disaster data. The primary backfill workflow includes all data sources: Resolver connectors (ACLED, IDMC, IFRC), resolution sources (FEWS NET IPC, IPC API, GDACS), structured data (conflict forecasts, ACAPS, ReliefWeb, ACLED political, NMME), and context sources (ENSO, seasonal TC, HDX Signals, CrisisWatch). NMME ingestion, previously a standalone workflow, is now a step within the structured data pipeline (with graceful handling for the fact that CPC FTP files are published around the 9th–10th of each month). All scheduled workflows run on independent schedules and write to the canonical database or cached JSON files that the HS pipeline reads at runtime. This decouples data refreshes from the HS run itself, ensuring that fresh data is available even between HS runs and that individual scraper or API failures do not block the main pipeline.
