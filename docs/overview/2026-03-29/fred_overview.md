...
This version reflects changes from PRs #643–#713, building on the per-hazard architecture introduced in PR #641. Key changes include significantly tighter grounding recency windows, lowered thresholds for adversarial checks and tail packs, likelihood-only RC levels, a redesigned bulk-fetch structured data ingestion pipeline, official deprecation of the question-level web research pipeline, source-priority steering in grounding prompts, model-aware calibration advice, a new ICG CrisisWatch connector with Playwright-based scraping, binary event forecasts (EVENT_OCCURRENCE), FEWS NET IPC Phase 3+ integration, GDACS as a Resolver connector, structured reasoning traces, LLM transcript viewing, DuckDB memory management fixes, DB-first context data loading, a consolidated single-job backfill workflow, epoch-specific question IDs, and CrisisWatch injection into SPD prompts.

- **Tighter grounding recency windows.** RC grounding recency windows have been substantially tightened to focus on very recent disruptions: ACE now uses 14 days (previously 90), DR uses 30 days (previously 120), FL uses 14 days (previously 90), HW uses 14 days (previously 90), and TC uses 30 days (previously 120). Triage grounding windows were also tightened: ACE uses 60 days (previously 120), DR uses 90 days (previously 180), FL uses 60 days (previously 120), HW uses 60 days (previously 120), and TC uses 60 days (previously 120). The intent is that RC grounding hunts for very recent change signals, while triage grounding captures a somewhat wider operational picture.
...
Resolver connectors pull from external providers. In a February 2026 refactoring, seven defunct or disused connectors (including EM-DAT, DTM, IPC, ReliefWeb, UNHCR, WFP, and WHO) were removed and the system was consolidated behind a formal connector protocol. Five active connectors now feed the Resolver: ACLED (conflict event and fatality records), IDMC (internal displacement flows via the Helix API), IFRC Montandon (natural-hazard people-affected data via the IFRC GO API, replacing the former EM-DAT connector), GDACS (disaster population exposure for floods, droughts, and tropical cyclones), and FEWS NET IPC (food security Phase 3+ population estimates for 48 monitored countries). IFRC GO provides structured impact data across multiple metrics (affected, fatalities, injured, displaced, missing) and maps disaster types to hazard codes automatically using IFRC's own disaster-type classification. GDACS writes two metrics per country-month-hazard: `in_need` (population exposed, used for PA-based forecasts) and `event_occurrence` (a binary 1/0 indicator based on alert level, where Orange or Red alerts count as events and Green alerts do not). The `event_occurrence` metric supports binary forecast questions that ask whether a significant disaster event will occur, complementing the SPD-based forecasts that estimate impact magnitude. FEWS NET IPC writes `phase3plus_in_need` (current situation, used for resolution) and `phase3plus_projection` (most likely scenario, used as context in forecaster prompts).
...
**FEWS NET IPC Phase 3+ Population Estimates.** The Famine Early Warning Systems Network (FEWS NET), operated by USAID, publishes IPC-compatible food security phase classifications for 48 monitored countries. Fred fetches population estimates from the FEWS NET Data Warehouse, focusing on two scenarios: Current Situation (used for forecast resolution and scoring) and Most Likely (used as forward-looking context in prompts). For each country and analysis period, the system stores the population classified at IPC Phase 3 or above ("Crisis or worse"), which is the primary metric for the PHASE3PLUS_IN_NEED forecast metric. FEWS NET data is used for drought (DR) hazard questions: in FEWS NET-monitored countries, DR questions use PHASE3PLUS_IN_NEED as their forecast metric rather than people affected (PA), because FEWS NET Phase 3+ provides more precise and operationally relevant ground truth for food-security-driven crises. The system maintains a list of FEWS NET-monitored country ISO3 codes, and DR/PA questions are blocked for non-monitored countries to prevent unresolvable forecasts.

**IPC Phase Classifications (legacy).** The IPC API connector exists in the codebase but is currently inactive because no API key is available. The `ipc_phases` table is always empty. FEWS NET Phase 3+ data (described above) serves as the primary food security data source. If an IPC API key becomes available in the future, both sources could be used in parallel.
...
| IPC phases (legacy, inactive) | DR | All | All |
...
Scheduled workflows include: one for refreshing the ENSO state cache (scraping IRI/CPC), one for refreshing the seasonal TC forecast cache (running the TSR, NOAA CPC, and BoM scrapers), one for refreshing ICG CrisisWatch data (Playwright-based monthly scraper), and a structured data ingestion workflow that bulk-fetches all humanitarian monitoring feeds (ACAPS, IPC, ReliefWeb) and NMME seasonal forecasts. The structured data ingestion was redesigned from per-country sequential API calls to bulk global fetches with concurrent processing, reducing runtime from over 6 hours to approximately 5–10 minutes. NMME ingestion, previously a standalone workflow, is now a step within the structured data ingestion workflow (with graceful handling for the fact that CPC FTP files are published around the 9th–10th of each month). All scheduled workflows run on independent schedules and write to the canonical database or cached JSON files that the HS pipeline reads at runtime. This decouples data refreshes from the HS run itself, ensuring that fresh data is available even between HS runs and that individual scraper or API failures do not block the main pipeline.
