...
This version reflects changes from PRs #643–#670, building on the per-hazard architecture introduced in PR #641. Key changes include significantly tighter grounding recency windows, lowered thresholds for adversarial checks and tail packs, likelihood-only RC levels, a redesigned bulk-fetch structured data ingestion pipeline, official deprecation of the question-level web research pipeline, source-priority steering in grounding prompts, model-aware calibration advice, and a new ICG CrisisWatch connector with Playwright-based scraping.
