...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        metavar="CONNECTOR=MODE",
        help=(
            "Force a connector to run with a specific mode by prepending '--mode <MODE>'. "
            "Useful for workflows that need records vs header-only overrides."
        ),
    )
    if argv is None:
        argv = []
...
def _parse_force_modes(values: Sequence[str]) -> Dict[str, str]:
    mapping: Dict[str, str] = {}
    for raw in values:
        text = (raw or "").strip()
        if not text or "=" not in text:
            continue
        connector, mode = text.split("=", 1)
        key = connector.strip()
        value = mode.strip()
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    force_modes = _parse_force_modes(args.force_mode)
    root = Path.cwd()
    ...
    for connector in connectors:
        ...
        additional = list(extra_args.get(name, []))
        forced = force_modes.get(name)
        if forced and not any(arg == "--mode" or arg.startswith("--mode=") for arg in additional):
            cmd.extend(["--mode", forced])
        if additional:
