...
API_BASE = os.getenv("PYTHIA_API_BASE", "http://localhost:8080/v1")
API_TOKEN = st.secrets.get("pythia_api_token")  # set in .streamlit/secrets.toml
...
def render_forecaster_dashboard():
...
st.title("Pythia Operator Console")

tab_run, tab_hs, tab_fc, tab_res, tab_calib, tab_rank = st.tabs(
    [
        "▶️ Start Pythia",
        "🛰️ Horizon Scanner",
        "🤖 Forecaster",
...
with tab_run:
    st.subheader("Launch a new Pythia run")
    countries = st.multiselect(
        "Countries (ISO3)",
        options=["KEN", "SOM", "ETH", "SDN", "UGA"],
        placeholder="Type ISO3...",
        key="run_countries",
    )
    if st.button("Start", key="start_run"):
        try:
            resp = _api_post("/run", {"countries": countries})
            st.success(f"Run accepted: {resp.get('run_id')}. Check other tabs for outputs.")
        except Exception as exc:  # pragma: no cover - Streamlit runtime feedback
            st.error(f"Failed to start run: {exc}")


with tab_hs:
    st.subheader("Horizon Scanner narratives")
    iso3 = st.text_input("ISO3", "KEN", key="hs_iso3")
    month = st.text_input("Target month (YYYY-MM)", "2025-11", key="hs_month")
    if iso3 and month:
        try:
            data = _api_get("/questions", {"iso3": iso3, "month": month})
            st.dataframe(pd.DataFrame(data.get("rows", [])))
        except Exception as exc:
            st.error(f"Failed to load horizon scanner questions: {exc}")


with tab_fc:
    st.subheader("Forecast ensembles")
    iso3_fc = st.text_input("ISO3 (forecasts)", "KEN", key="fc_iso3")
    month_fc = st.text_input("Target month (YYYY-MM)", "2025-11", key="fc_month")
    if iso3_fc and month_fc:
        try:
            data = _api_get(
                "/forecasts/ensemble",
                {"iso3": iso3_fc, "target_month": month_fc, "metric": "PIN"},
            )
            st.dataframe(pd.DataFrame(data.get("rows", [])))
        except Exception as exc:
    ...
    st.divider()
    render_forecaster_dashboard()


with tab_res:
    st.subheader("Resolver outcomes")
    iso3_res = st.text_input("ISO3 (resolutions)", "KEN", key="res_iso3")
    month_res = st.text_input("Month (YYYY-MM)", "2025-11", key="res_month")
    if iso3_res and month_res:
        try:
            data = _api_get("/resolutions", {"iso3": iso3_res, "month": month_res, "metric": "PIN"})
            st.dataframe(pd.DataFrame(data.get("rows", [])))
        except Exception as exc:
            st.error(f"Failed to load resolver data: {exc}")


with tab_calib:
    st.subheader("Calibration insights")
    st.info("Detailed calibration diagnostics live in the Forecaster tab above.")


with tab_rank:
    st.subheader("PIN rankings")
    month_rank = st.text_input("Target month (YYYY-MM)", "2025-11", key="rank_month")
    normalize = st.checkbox("Normalize by population", True, key="rank_normalize")
    if month_rank:
        try:
            data = _api_get(
                "/rankings",
            ...
            st.error(f"Failed to load rankings: {exc}")
