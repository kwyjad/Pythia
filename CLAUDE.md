...
- **Calibration advice dual-constraint failures** (fixed): `_upsert_advice()` in `pythia/tools/generate_calibration_advice.py` failed in two ways due to the `calibration_advice` table having both the original 3-column PK `(as_of_month, hazard_code, metric)` and the newer 4-column unique index `ux_calibration_advice (as_of_month, hazard_code, metric, model_name)` — the schema migration's `DROP CONSTRAINT` silently failed on pre-existing DBs. First failure: `BinderException` from `INSERT OR REPLACE` when DuckDB couldn't pick a conflict target. Second failure: `ConstraintException` violating the 3-column PK when writing a second `model_name` for the same `(month, hazard, metric)`. Fixed in three places: (1) `_upsert_advice` now proactively drops the legacy 3-column PK and ensures the 4-column unique index on first call (once per process, guarded by `_PK_DROPPED` flag), (2) uses `INSERT INTO ... ON CONFLICT (as_of_month, hazard_code, metric, model_name) DO UPDATE SET ...`, (3) schema migration tries multiple PK constraint names.
