...
- **Calibration advice dual-constraint failures** (fixed): `_upsert_advice()` in `pythia/tools/generate_calibration_advice.py` failed repeatedly due to the `calibration_advice` table having both the original 3-column PK `(as_of_month, hazard_code, metric)` and the newer 4-column unique index `ux_calibration_advice (as_of_month, hazard_code, metric, model_name)`. Root cause: the `CREATE TABLE` DDL in `schema.py` still defined the 3-column PK, and the `DROP CONSTRAINT` migration silently failed because DuckDB auto-generates constraint names that varied by version — hardcoded names like `calibration_advice_pkey` never matched. Fixed comprehensively: (1) changed `CREATE TABLE` DDL in `schema.py` to `PRIMARY KEY (as_of_month, hazard_code, metric, model_name)` so fresh DBs never get the old PK, (2) `_migrate_calibration_advice_pk()` now queries `information_schema.table_constraints` to discover actual constraint names rather than guessing, then drops any PK/UNIQUE constraints that aren't `ux_calibration_advice`, (3) `_upsert_advice` uses `INSERT INTO ... ON CONFLICT (as_of_month, hazard_code, metric, model_name) DO UPDATE SET ...` without a preceding `DELETE` (the `ON CONFLICT` handles upsert), (4) `_seed_default_advice` now calls the migration and uses `ON CONFLICT ... DO NOTHING`, (5) both `schema.py` and `generate_calibration_advice.py` retain hardcoded fallback constraint names as belt-and-suspenders.
