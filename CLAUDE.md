...
- `ifrc_montandon` — IFRC GO connector (active; pulls Field Reports via `resolver/ingestion/ifrc_go_client.py`, writes `publisher='IFRC'` rows to `facts_resolved`). Tier-0 source for natural-hazard PA (FL, DR, TC). Field Reports often have `fatalities`/`injured` but not `affected` for the same event, which limits PA-resolution coverage — see `scripts/diagnose_pa_resolution_coverage.py` for the per-(hazard, metric) gap. **PA imputation** (in `extract_all_metrics` via `_maybe_derive_affected`): when a record has no `num_affected` but at least 2 of `{fatalities, injured, displaced, missing}` are present, the connector synthesises an `affected` row as the sum of those collateral metrics. Derived rows are written with `method='derived'`, `confidence='low'`, an event_id suffix `-derived`, and a `definition_text` that explicitly says "Derived ... as the sum of ..." so auditors can filter them out. The derived value is a floor estimate — IFRC's headline "affected" is usually broader than the sum, but the sum is the most defensible reconstruction when the primary field is absent.
