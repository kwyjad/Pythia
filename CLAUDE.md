...
- **Calibration advice ConstraintException on re-run** (fixed): `_upsert_advice()` in `pythia/tools/generate_calibration_advice.py` used `INSERT INTO calibration_advice`, which failed with a `ConstraintException` when the same PK `(as_of_month, hazard_code, metric)` already existed from a prior run. Fixed by changing to `INSERT OR REPLACE INTO`. The seeding function's separate `INSERT INTO` was left as-is because it already has a `SELECT 1` existence guard.
