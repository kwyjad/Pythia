...
- **Scoring**: `compute_scores` scores each (question, horizon_m) pair separately. SPD questions (PA, FATALITIES) get multiclass Brier (range **0–2**), log loss, and CRPS. Binary questions (EVENT_OCCURRENCE) get single Brier score (range **0–1**): `(forecast_p - outcome)^2` where forecast_p = bucket_1 probability from ensemble. PHASE3PLUS_IN_NEED uses SPD scoring. Missing resolution horizons are naturally excluded via JOIN. **Brier-family separation**: binary Brier (0–1, structurally near-zero for rare events correctly forecast as unlikely) and multiclass SPD Brier (0–2) are on different scales and must **never** be averaged together. Aggregations expose a `score_family` (`binary` / `spd`) so consumers can keep them apart — a naive mean across both families (e.g. averaging an `ensemble_mean` scores CSV that mixes 37 SPD + 24 binary questions) is meaningless and was the cause of the 0.200-vs-0.6237 dashboard/CSV discrepancy.
...
- **Binary NULL-model ensemble score was nondeterministic** (fixed): the binary path in `compute_scores.py` read the ensemble P(yes) from `forecasts_ensemble` with no `model_name` filter, ordered only by `created_at` — so when both `ensemble_mean_v2` and `ensemble_bayesmc_v2` rows existed, the NULL-model score row could be either. Now filtered to `model_name IN ('ensemble_mean_v2','track2_flash','ensemble')` with explicit preference order (mean first), matching the documented "binary is scored under the mean ensemble" convention.
