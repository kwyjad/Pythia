...
- **Calibration advice BinderException on upsert** (fixed): `_upsert_advice()` in `pythia/tools/generate_calibration_advice.py` used `INSERT OR REPLACE INTO calibration_advice`, which failed with `BinderException: Conflict target has to be provided for a DO UPDATE operation when the table has multiple UNIQUE/PRIMARY KEY constraints`. The table had both the original 3-column PK `(as_of_month, hazard_code, metric)` and the newer 4-column unique index `ux_calibration_advice (as_of_month, hazard_code, metric, model_name)` because the schema migration's `DROP CONSTRAINT` silently failed. Fixed in two places: (1) changed `INSERT OR REPLACE` to `INSERT INTO ... ON CONFLICT (as_of_month, hazard_code, metric, model_name) DO UPDATE SET ...` with explicit conflict target, (2) made the schema migration in `_ensure_calibration_advice_table` try multiple PK constraint names to ensure the old 3-column PK is dropped.
