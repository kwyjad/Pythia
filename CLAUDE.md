...
- `docs/audit_2026-07.md` — July 2026 full-codebase audit: what was fixed, deferred recommendations, and investigated-but-NOT-bugs (check it before re-reporting a previously-assessed issue)
...
- **Tier entries must match the lowercased `publisher` values connectors write** (`run_pipeline` uses publisher as the precedence source id) — tier 0 lists both `ifrc` (what the connector writes) and the legacy `ifrc_montandon` slug. The YAML also carries `tiebreak: [as_of_desc, value_nonnull, source_alpha]` and `defaults: {require_nonnegative, prefer_full_row_coverage}` — these are what `resolve_facts_frame` (the pipeline path) actually reads; the `metric_preference`/`source_precedence`/`conflict_rule` keys are consumed only by the legacy CLI path. When adding a connector whose rows can collide with tiered sources on the same (iso3, hazard, ym, metric), add its publisher slug to a tier.
