...
- **Bucket thresholds/labels are single-sourced** from `pythia/buckets.py` (`thresholds_for` / `interior_thresholds_for` / `labels_for`, derived from `BUCKET_SPECS`); `compute_scores.py` and `forecaster/scoring.py` expose their legacy constant names as derived values. Do not reintroduce literal threshold lists.
