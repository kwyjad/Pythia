...
- [`resolver/ingestion`](resolver/ingestion): Source-specific connectors, retry helpers, and offline stubs. Each client reads `resolver/ingestion/config/*.yml` for API parameters and writes staging CSV/Parquet under `resolver/staging/`. The orchestrator [`run_all_stubs.py`](resolver/ingestion/run_all_stubs.py) applies smoke defaults and per-source feature flags. The refreshed [`resolver/ingestion/idmc/`](resolver/ingestion/idmc) package keeps fixtures for fast tests while adding an online-capable client, HTTP helper, cache, reachability probe, and CLI toggles to fetch and memoize IDU payloads safely. Shared connector contracts live under [`resolver/ingestion/tests/contracts/`](resolver/ingestion/tests/contracts).【F:resolver/ingestion/idmc/client.py†L1-L207】【F:resolver/ingestion/idmc/http.py†L1-L99】【F:resolver/ingestion/idmc/cache.py†L1-L77】【F:resolver/ingestion/idmc/cli.py†L11-L87】【F:resolver/ingestion/idmc/probe.py†L1-L99】
...
- [`resolver/snapshot`](resolver/snapshot): DB-backed monthly snapshot builder (`facts_snapshot`) that consolidates canonical tables (e.g., `facts_resolved`, `facts_deltas`, `acled_monthly_fatalities`) for downstream consumers. CLI orchestration lives at [`resolver/cli/snapshot_from_db.py`](resolver/cli/snapshot_from_db.py).
...
- Continuous integration: `.github/workflows/resolver-ci.yml` includes a dedicated `tests (db backend)` job that installs the DuckDB extra and runs the full suite alongside the existing file-mode smoke tests.
