...
- Every resolver workflow proactively uninstalls any cached wheel and runs `pip install -e .` so tests import the workspace copy of `resolver`. Immediately before the pytest steps we install the `db`/`test` extras (falling back to `pip install duckdb pytest` if the extras are unavailable) and log a positive `import duckdb` probe so CI failures never stem from missing DuckDB wheels. After every pytest invocation—regardless of success—we generate `.ci/diagnostics/SUMMARY.md` via `scripts/ci/make_ai_summary.py`, capturing run metadata, environment details, pytest failures, resolver logs, and DuckDB schema snippets inlined for AI triage. The diagnostics folder always carries that summary (plus `pytest-junit.xml` when present) and we collect lightweight log/database snippets for convenience. Each job uploads both the folder and a `diagnostics_${JOB_NAME}_${DUCKDB_LABEL}.tar.gz` bundle as `diagnostics-<job>-<duckdb>-run<attempt>` with `overwrite: true` so retries never collide with prior artifacts, and a follow-up aggregation job merges every per-job summary into `diagnostics-consolidated-<run_id>-<attempt>/SUMMARY.md` for a single run-level view.
...
- `resolver-smoke.yml` runs on pushes, pull requests, and manual dispatches. It executes the full resolver pipeline end-to-end using offline stubs: `run_all_stubs.py --smoke` writes raw CSVs into `data/staging/<period>/raw`, `resolver.transform.normalize` canonically normalizes every adapter-supported source, and `resolver.tools.load_and_derive` loads DuckDB, resolves precedence, derives deltas (with negative swings preserved), and exports header-safe Parquet snapshots. The job asserts that `facts_resolved.parquet` exists and uploads all Parquet files from the run as the `resolver-smoke-snapshots` artifact so PRs can prove the wiring without any network access.
...
Every job in `resolver-ci-fast` and `resolver-ci` uploads a `diagnostics-<job>-<duckdb>-run<attempt>` artifact containing the raw `.ci/diagnostics/` folder (including the AI-focused `SUMMARY.md` and any `pytest-junit.xml` file) alongside a sibling `diagnostics_<job>_<duckdb_label>.tar.gz` archive for quick download. For example, the fast workflow publishes `diagnostics-fast-tests-default-run1` on the first attempt. The upload step always runs with `overwrite: true`, so retries never hit 409 conflicts, and the tarball captures any DuckDB databases, log files, and the `pytest-junit.xml` produced during the run—even when pytest fails. A final aggregate job downloads all per-job artifacts and uploads `diagnostics-consolidated-<run_id>-<attempt>/SUMMARY.md`, which concatenates every job summary into a single file for AI triage.
