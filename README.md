...
- Every resolver workflow proactively uninstalls any cached wheel and runs `pip install -e .` so tests import the workspace copy of `resolver`. Immediately before the pytest steps we install the `db`/`test` extras (falling back to `pip install duckdb pytest` if the extras are unavailable) and log a positive `import duckdb` probe so CI failures never stem from missing DuckDB wheels. After every pytest invocation—regardless of success—we populate `.ci/diagnostics/` with `pytest-junit.xml` (when present) and any resolver logs or DuckDB artifacts tracked by git, then create a `diagnostics_${JOB_NAME}_${DUCKDB_LABEL}.tar.gz` bundle alongside the folder. Each job uploads `diagnostics-<job>-<duckdb>-run<attempt>` with those assets attached and enables `overwrite` so retries never collide with prior artifacts.
...
Every job in `resolver-ci-fast` and `resolver-ci` uploads a `diagnostics-<job>-<duckdb>-run<attempt>` artifact containing two things: (1) the raw `.ci/diagnostics/` folder populated during the run, and (2) a sibling `diagnostics_<job>_<duckdb_label>.tar.gz` archive for quick download. For example, the fast workflow publishes `diagnostics-fast-tests-default-run1` on the first attempt. The upload step always runs with `overwrite: true`, so retries never hit 409 conflicts, and the tarball captures any DuckDB databases, log files, and the `pytest-junit.xml` produced during the run—even when pytest fails.
