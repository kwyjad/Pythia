...
- **Codex users:** run `bash tools/codex_run_fast_tests.sh` to install the DuckDB/PyArrow/pandas test dependencies and execute the fast suite. The helper script sets safe defaults for offline execution, and `resolver/tests/conftest.py` ensures `RESOLVER_SKIP_DTM=1` unless you explicitly opt into online connectors.
