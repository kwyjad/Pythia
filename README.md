...
- Runtime discovery snapshots and deep diagnostics now live in dedicated folders that the CI workflow uploads as separate artifacts:
  ...
  - Discovery retries use 5s connect and 30s read timeouts and automatically fall back to `operations-list` and a bundled ISO-3 roster when the primary endpoint times out. Every run writes `diagnostics/ingestion/dtm/discovery_fail.json` summarizing each stage and `diagnostics/ingestion/metrics/metrics.json` with attempt counts, rows, and elapsed time so CI jobs that fail early still upload structured artifacts.
