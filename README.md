...
- Every resolver workflow proactively uninstalls any cached wheel and runs `pip install -e .` so tests import the workspace copy of `resolver`. The jobs print the merge commit SHA, PR head (when applicable), the active Python executable, resolved module paths, DuckDB version, and a SHA-256 digest of `duckdb_io.py`. The exact sources used for the run (including `duckdb_io.py` and `schema.sql` when present) are uploaded as uniquely named artifacts following the pattern `<job>-<duckdb_label>-<run_id>-<run_attempt>-<tag>.zip`, guaranteeing that concurrent attempts never collide.
