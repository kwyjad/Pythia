...
- Every resolver workflow proactively uninstalls any cached wheel and runs `pip install -e .` so tests import the workspace copy of `resolver`. The jobs print the merge commit SHA, PR head (when applicable), the active Python executable, resolved module paths, DuckDB version, and a SHA-256 digest of `duckdb_io.py`. After the tests finish they invoke `python -m resolver.tools.ci_diag_bundle` to capture those stamps, relevant environment variables, file hashes, DuckDB schema snapshots, row counts, and the `pytest-junit.xml` report in a single `diagnostics.zip`. That bundle is uploaded once per job as `diagnostics-bundle-<run_id>-<run_attempt>-<suite>-<duckdb>` so retries overwrite the prior attempt instead of colliding.
...
### CI diagnostics bundle

Every CI job emits a `diagnostics-bundle-*` artifact generated by `resolver.tools.ci_diag_bundle`. The zip contains:

- `meta.txt` with the commit SHA, PR head (when available), job name, DuckDB version, and creation timestamp.
- `env.json` capturing the GitHub/matrix environment variables, Python executable, resolver-related flags, and the DuckDB package listing.
- Per-file SHA256 stamps for `resolver/db/duckdb_io.py`, export tooling, query readers, and `schema.sql` when present.
- DuckDB schema snapshots (tables, `PRAGMA table_info`, named indexes, row counts, and optional `ym=` slices pulled from env vars).
- `pytest-junit.xml` plus a compact failure summary so you can see the first line of each failing test without unpacking the XML by hand.

When you need help debugging a CI failure, attach that diagnostics bundle to your issue or share it with the resolver maintainers—everything they need (schema, counts, hashes, junit) is already inside.
