...
- **Batch-API staged pipeline (opt-in)**: The HS and forecaster LLM families can run through provider Batch APIs at 50% off both input and output tokens. `python -m horizon_scanner.horizon_scanner --stage {hs_submit,hs_rc_collect,hs_finalize}` and `python -m forecaster.cli --phase {submit,collect}` split each family into a submit wave (prompts built + enqueued in [`pythia/llm_batch.py`](pythia/llm_batch.py), byte-identical bodies to the sync path) and a collect wave (results replayed, per-item sync fallback on any miss). GitHub orchestration: `pythia_pipeline_stage.yml` (staged runs) + `poll_llm_batches.yml` (15-min poller that advances the pipeline when batches finish). Everything defaults OFF (`PYTHIA_BATCH_API_ENABLED=0`); the default `--stage full` / `--phase full` paths are unchanged. Related: static-first prompt ordering + provider prompt caching behind `PYTHIA_PROMPT_V3_ORDER` / `PYTHIA_PROMPT_CACHE_ENABLED` (also default OFF, gated on an A/B validation run via [`scripts/compare_prompt_order_runs.py`](scripts/compare_prompt_order_runs.py)).
