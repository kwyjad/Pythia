...
- The DTM connector performs a configuration preflight and mirrors the results to `diagnostics/ingestion/dtm_config_issues.json`. When the log level is `DEBUG`, it also snapshots the resolved source list to `diagnostics/ingestion/dtm_sources_resolved.json` so you can reconcile IDs, inferred columns, and skip reasons locally. Pass `--fail-on-missing-config` (or set `DTM_STRICT=1`) to make the run exit with code `2` whenever the preflight discovers missing `id_or_path` values.
- Runtime discovery snapshots and deep diagnostics now live in dedicated folders that the CI workflow uploads as separate artifacts:
  - `diagnostics/ingestion/raw/dtm_countries.json` captures the payload returned by `DTMApi.get_all_countries()`.
  - Discovery retries use 5s connect and 30s read timeouts and automatically fall back to `operations-list` and a bundled ISO-3 roster when the primary endpoint times out. Providing `api.countries` with a non-empty list bypasses network discovery entirely and uses that explicit ordering. Every run writes `diagnostics/ingestion/dtm/discovery_fail.json` summarizing each stage and `diagnostics/ingestion/metrics/metrics.json` with attempt counts, rows, and elapsed time so CI jobs that fail early still upload structured artifacts. The static roster (`resolver/ingestion/static/iso3_master.csv`) is RFC 4180-compliant—quote any country names that contain commas and run `python resolver/ingestion/static/iso3_master.validate.py` before committing edits.
  - `diagnostics/ingestion/metrics/dtm_per_country.jsonl` records per-country/per-level row counts and elapsed timings.
  - `diagnostics/ingestion/samples/dtm_sample.csv` stores up to 200 normalized rows across all requests, while `diagnostics/ingestion/logs/dtm_client.log` mirrors the enriched run log (window, country counts, masked key suffix, retry stats).
  - The DTM client now logs `api_base_url`, `resolved_country_count`, `resolved_admin_levels`, `date_window`, and a sanitized `request_params_clean` payload before the first fetch so smoke jobs can confirm configuration without network access.
  - When zero rows are returned the diagnostics include a `zero_rows_reason` (`empty_country_list`, `invalid_indicator`, `api_empty_response`, or `filter_excluded_all`). The CLI exits `0` in this case unless `--strict-empty` (or `DTM_STRICT_EMPTY=1`) is set, in which case it exits `2`. Offline smoke runs (`--offline-smoke` or `DTM_OFFLINE_SMOKE=1`) emit a header-only CSV, print a one-line `offline-smoke mode active` message, and exit `0` without requiring `DTM_API_KEY`.
...
- The connector still raises an error when discovery returns zero countries, but zero-row responses now finish with status **`ok-empty`** (exit `0`) unless `--strict-empty` is enabled. Historical diagnostics that expected **`ok-empty`** runs remain available for reference, and the reason field continues to record messages such as `DTM: All requests returned 0 rows in window 2023-07-01..2024-06-30`.
- Each invocation emits `diagnostics/ingestion/dtm_run.json`, which records the ingestion window, per-source stats (`rows_before`, `rows_after`, `parse_errors`, `min`, `max`), the resolved outputs, and overall totals. The summary table populates the Kept/Dropped/ParseErrs cells directly from this JSON so you can pivot from the dashboard to the raw diagnostics without guessing.
- The CSV companion `data/staging/dtm_displacement.csv.meta.json` mirrors the output row count and now also captures `backfill_start`, `backfill_end`, and the source counts (`sources_total`, `sources_valid`, `sources_invalid`).
- To sanity-check a source without the ingestion window, run `python -m resolver.ingestion.dtm_client --no-date-filter` locally or trigger the backfill workflow with `no_date_filter=true`. The connector still logs the configured window, but keeps every parsed row so you can confirm the upstream sheet is populated before reenabling the filter.
- When you are bringing new sources online, trigger the initial backfill workflow with `empty_policy=fail` (or run `python -m resolver.ingestion.dtm_client --strict-empty`) so header-only runs exit with code `2`. This keeps the workflow honest without sacrificing the diagnostics artifacts you need for triage. Once the configuration is stable you can switch `empty_policy` back to `allow`.
