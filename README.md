...
Global CLI flags such as `--period` must appear **before** the positional arguments when invoking `resolver.tools.load_and_derive`. All workflows now use the single-shot form so contributors run one command locally and CI produces identical diagnostics:

```bash
python -m resolver.tools.load_and_derive --period "${PERIOD_LABEL}" \
  --staging-root data/staging --snapshots-root data/snapshots \
  --db data/resolver.duckdb --allow-negatives 1
```

`PeriodMonths.from_label` accepts canonical quarterly strings such as `2025Q4`. CI-friendly aliases that start with `ci`, `dev`, or `test` (for example, `ci-smoke`) are automatically mapped to the current UTC quarter so smoke runs stay ergonomic while the loader still receives a valid `YYYYQ#` label. Running a global flag after the command (for example, `python -m resolver.tools.load_and_derive load-canonical --period ci-smoke`) still triggers argparse’s “the following arguments are required: --period” error.

### Diagnostics in CI

All resolver workflows now execute critical commands through `scripts/ci/run_and_capture.sh`, a tiny wrapper that mirrors stdout and stderr to `.ci/diagnostics/<step>.out.log` / `.err.log`, emits a combined `.tail.txt` for quick triage, and writes the corresponding status code to `.ci/exitcodes/<step>`. Fast tests, smoke runs, and nightly jobs therefore emit a uniform diagnostics bundle with:

- `pip-freeze.txt` captured via `scripts/ci/run_and_capture.sh pip-freeze "pip freeze | tee .ci/diagnostics/pip-freeze.txt"` so package mismatches are visible without rerunning the job, while still streaming the package list into the log tail.
- Canonical CSV/Parquet summaries generated by `python scripts/ci/list_canonical.py --dir <canonical_dir> --out .ci/diagnostics/canonical-listing.txt`, which notes missing directories, file sizes, header-safe row totals, and flags any files whose row counts could not be inferred.
- DuckDB table statistics written by `python scripts/ci/db_counts.py --db data/resolver.duckdb --out .ci/diagnostics/duckdb-counts.txt`, which now enumerates every `facts_*` table alongside the core trio (`facts_raw`, `facts_resolved`, `facts_monthly_deltas`) and records any count errors directly in the JSON payload.
...
python scripts/ci/list_canonical.py --dir "data/staging/${PERIOD_LABEL}/canonical" --out .ci/diagnostics/canonical-listing.txt
python scripts/ci/db_counts.py --db data/resolver.duckdb --out .ci/diagnostics/duckdb-counts.txt
```

All workflows upload `.ci/diagnostics/**` and `.ci/exitcodes/**` so the aggregated `SUMMARY.md` produced by `.github/actions/collect-diagnostics` has consistent log attachments, command tails, pip-freeze snapshots, canonical totals, DuckDB counts, pytest results, and exit-code breadcrumbs.
