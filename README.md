...
- The ingestion summary (`diagnostics/ingestion/summary.md`) now highlights the resolved DTM configuration, staging readiness (counts and sizes for both `resolver/staging` and the legacy `data/staging`), zero-row root causes (primary reason plus top drop reasons), selector effectiveness, and reachability diagnostics generated by `scripts/ci.probe_dtm_reachability` (DNS, TCP/TLS timings, HTTP `curl -I`, CA bundle, and current egress IPs) ahead of the per-connector table and deep-dive sections.
