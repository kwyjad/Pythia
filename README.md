...
The IDMC (IDU) connector targets the live IDU API and now expects a concrete date window. Supply `--start`/`--end` on the CLI (or export `RESOLVER_START_ISO`/`RESOLVER_END_ISO`, falling back to `BACKFILL_*`) to wire a backfill span into the client; `--no-date-filter` still disables filtering when required. Diagnostics and the `why_zero.json` payload capture the window source, request counts, and staging row totals so empty runs explain whether the API returned zero features or the fetch never reached the network.【F:resolver/ingestion/idmc/cli.py†L300-L384】【F:resolver/ingestion/idmc/cli.py†L520-L611】【F:resolver/ingestion/idmc/cli.py†L900-L992】【F:resolver/ingestion/idmc/client.py†L516-L610】 When `countries: []` (or the field is omitted) in `ingestion/config/idmc.yml`, the client targets the full ISO3 roster from `resolver/ingestion/static/iso3_master.csv`; override that scope via `IDMC_COUNTRIES`, `IDMC_COUNTRIES_FILE`, or `--only-countries` and the selection propagates through the client and diagnostics.【F:resolver/ingestion/utils/country_utils.py†L118-L209】【F:resolver/ingestion/idmc/cli.py†L223-L323】【F:resolver/ingestion/idmc/client.py†L520-L575】 Each run writes `diagnostics/ingestion/idmc/summary.md` with the resolved window, country sample, and staging counts alongside `diagnostics/ingestion/idmc/why_zero.json` for zero-row investigations.【F:resolver/ingestion/idmc/cli.py†L60-L122】【F:resolver/diagnostics/templates/idmc_summary.md.j2†L1-L21】【F:resolver/ingestion/idmc/cli.py†L900-L992】 The live client now emits PostgREST filters (`displacement_start_date=gte.*`, `displacement_end_date=lte.*`) and batches ISO3 codes to respect URL limits; enable the mirrored HDX CSV rescue explicitly with `--allow-hdx-fallback` (or `IDMC_ALLOW_HDX_FALLBACK=1`) when you need it and diagnostics record each `http_get` attempt alongside any fallback rows that were staged.【F:resolver/ingestion/idmc/cli.py†L212-L320】【F:resolver/ingestion/idmc/cli.py†L388-L471】【F:resolver/ingestion/idmc/client.py†L506-L719】【F:resolver/ingestion/idmc/client.py†L958-L1362】
Live runs also respect explicit HTTP tuning. The connector and the CI reachability probe advertise a configurable `User-Agent` (from `IDMC_USER_AGENT` or `RELIEFWEB_USER_AGENT`) and accept overrides for connect/read timeouts with `IDMC_HTTP_CONNECT_TIMEOUT_S` and `IDMC_HTTP_READ_TIMEOUT_S`; defaults remain 5 s/25 s and show up in diagnostics alongside timeout counters so soft failures never masquerade as green-but-empty runs.【F:resolver/ingestion/idmc/http.py†L15-L253】【F:resolver/ingestion/idmc/client.py†L560-L1112】【F:scripts/ci/probe_idmc_reachability.py†L1-L210】 Per-chunk attempt lines, the reachability bullet list, and the fallback outcome now live in `diagnostics/ingestion/idmc/summary.md`, making workflow reviews highlight which URLs timed out, which returned 2xx, and whether HDX rescued the batch.【F:resolver/diagnostics/templates/idmc_summary.md.j2†L1-L27】【F:resolver/ingestion/idmc/cli.py†L1096-L1280】 CI enables `--allow-hdx-fallback` explicitly so backfills continue to land rows while fast tests keep the behaviour off by default for determinism.【F:.github/workflows/resolver-initial-backfill.yml†L205-L218】
...
Enable export output with `--enable-export --write-outputs --series flow,stock` (already wired into the CI backfill) to write `resolver/staging/idmc/flow.csv` and canonical facts artifacts for the `idp_displacement_new_idmc` metric; the exporter now maps that staging file directly.【F:resolver/ingestion/idmc/cli.py†L410-L478】【F:resolver/tools/export_config.yml†L70-L116】 Choose how the connector reaches IDMC by supplying `--network-mode {live,cache_only,fixture}` (defaults to `live`) or by exporting `IDMC_NETWORK_MODE` before you run the CLI; diagnostics record the effective mode together with per-class HTTP status counts (always the canonical `{"2xx","4xx","5xx"}` buckets), aggregated attempt timings, and any HDX fallbacks invoked during the run.【F:resolver/ingestion/idmc/cli.py†L178-L208】【F:resolver/ingestion/idmc/cli.py†L632-L740】【F:resolver/ingestion/idmc/client.py†L600-L736】 A typical staging run looks like `python -m resolver.ingestion.idmc.cli --network-mode live --start 2024-01-01 --end 2024-03-31 --enable-export --write-outputs --series flow,stock --debug`, while local cache rehearsals can rely on `IDMC_NETWORK_MODE=cache_only python -m resolver.ingestion.idmc.cli --start … --end … --write-outputs`.
