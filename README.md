...
- Every resolver workflow proactively uninstalls any cached wheel and runs `pip install -e .` so tests import the workspace copy of `resolver`. The jobs print the merge commit SHA, PR head (when applicable), the active Python executable, resolved module paths, DuckDB version, and a SHA-256 digest of `duckdb_io.py`. Fast-tests, offline connector smokes, and the db-backend matrices all emit per-job diagnostics bundles and the resolver-ci workflow stitches them together—along with the fast-tests artifacts produced by the sibling workflow—into a single `diagnostics-consolidated-<run_id>-<run_attempt>.zip` that contains the one “dream” `SUMMARY.md` for the entire run. Nightly legs reuse the same tooling when scheduled but add cache-disabled db-backend coverage on top.
...
- Quote any `name:` values that contain colons or `${{ ... }}` expressions and keep heredoc terminators aligned with their `cat <<'TAG'` lines. The dedicated **CI Lint** workflow (yamllint + actionlint) runs on pushes and pull requests to block malformed YAML before it reaches the main workflows.
...
The resolver-ci workflow uploads **exactly one** consolidated artifact named `diagnostics-consolidated-<run_id>-<run_attempt>.zip`. It is assembled from the per-job bundles produced by `python -m resolver.tools.ci_diag_bundle` across the fast-tests workflow, the db-backend matrix, and the connector smokes, so the top-level `SUMMARY.md` captures every suite in one place. Each bundle automatically scans the workspace `.ci/` cache alongside `/tmp` scratch space to capture any DuckDB databases materialised during the run. The “dream” summary records commit SHAs (merge and PR head), suite labels, DuckDB and Python versions, per-module SHA256 hashes and import paths, **auto-discovers every `*.duckdb` file created during the run**, records each table’s schema, unique indexes (with canonical column order), row counts, compact previews, declared-key PASS/FAIL verdicts, `ym` rollups, latest `as_of` probes, and a concise test failure digest with traceback snippets.

After each PR run, download `diagnostics-consolidated-<run_id>-<run_attempt>.zip`, open it locally, and paste the contents of the top-level `SUMMARY.md` into your issue or pull request comment. That single page contains everything maintainers need to diagnose CI failures—no extra logs or screenshots required.
