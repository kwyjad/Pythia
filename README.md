...
- Header-only runs now surface as **`ok-empty`** in the connector summary with reasons such as `header-only; kept=0` or `header-only; 2 sources invalid; kept=0`. The table adds **Kept**, **Dropped**, and **ParseErrs** columns so you can immediately see how many rows survived the window filter and how many were rejected during parsing.
