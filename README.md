...
- Every resolver workflow proactively uninstalls any cached wheel and runs `pip install -e .` so tests import the workspace copy of `resolver`. Immediately before the pytest steps we install the `db`/`test` extras (falling back to `pip install duckdb pytest` if the extras are unavailable) and log a positive `import duckdb` probe so CI failures never stem from missing DuckDB wheels. After every pytest invocation—regardless of success—we call `python scripts/ci/make_diagnostics.py`. The Python bundler sweeps the workspace for `pytest-junit.xml`, resolver logs, environment metadata, `pip freeze`, and any `*.duckdb` files under both the checkout and `/tmp/pytest-of-*`, then emits a `diagnostics_${JOB_NAME}_${DUCKDB_LABEL}.tar.gz` bundle alongside a mirrored `.ci/diagnostics/` tree and prints a summary of the captured files. Each job uploads `diagnostics-<job>-<duckdb>-run<attempt>` with those assets attached and enables `overwrite` so retries never collide with prior artifacts. The aggregate job simply downloads the tarballs, extracts them into `consolidated/`, and builds a run-level `SUMMARY.md` that inventories every captured file. Nightly legs reuse the same tooling when scheduled but add cache-disabled db-backend coverage on top.
...
Every job in `resolver-ci-fast` and `resolver-ci` uploads a `diagnostics-<job>-<duckdb>-run<attempt>` artifact containing two things: (1) the raw `.ci/diagnostics/` folder populated by `scripts/ci/make_diagnostics.py`, and (2) a sibling `diagnostics_<job>_<duckdb_label>.tar.gz` archive for quick download. The bundler prints a summary of the collected files so you can sanity-check the capture in-line, and the `overwrite: true` upload option prevents retries from hitting 409 conflicts. The aggregate job pulls those tarballs, extracts them into a `consolidated/` directory, and writes a synthetic `SUMMARY.md` that lists the captured files per job alongside the run and attempt identifiers. Because the bundler walks both the workspace and `/tmp/pytest-of-*`, any DuckDB databases, log files, and the `pytest-junit.xml` produced during the run are always preserved—even when pytest fails.

After each PR run, download the `diagnostics-consolidated-<run_id>-<run_attempt>` artifact, open `SUMMARY.md`, and paste its contents into your issue or pull request comment. That single page inventories every diagnostic asset so maintainers know exactly which tarball to expand when investigating a failure.
