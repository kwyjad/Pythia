...
- Every resolver workflow proactively uninstalls any cached wheel and runs `pip install -e .` so tests import the workspace copy of `resolver`. The jobs print the merge commit SHA, PR head (when applicable), the active Python executable, resolved module paths, DuckDB version, and a SHA-256 digest of `duckdb_io.py`. After the fast-test matrices finish they invoke `python -m resolver.tools.ci_diag_bundle` once per matrix entry and then aggregate those zips into a single `diagnostics-fast-tests-<run_id>-<run_attempt>.zip` with a top-level `SUMMARY.md`. Nightly database legs still run the same bundle generator directly when scheduled, but pushes/PRs only exercise the faster suite.
