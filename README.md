...
- Every resolver workflow proactively uninstalls any cached wheel and runs `pip install -e .` so tests import the workspace copy of `resolver`. Immediately before the pytest steps we install the `db`/`test` extras (falling back to `pip install duckdb pytest` if the extras are unavailable) and log a positive `import duckdb` probe so CI failures never stem from missing DuckDB wheels. After every pytest invocation—regardless of success—we still generate `.ci/diagnostics/SUMMARY.md` via `scripts/ci/make_ai_summary.py` for backwards compatibility, and the shared composite action at `.github/actions/collect-diagnostics` emits a top-level `SUMMARY.md` with run metadata, sanitized environment details, pytest results, resolver logs, and DuckDB table counts before packaging the underlying text snapshots into `dist/diagnostics-<job>-<run_id>-<attempt>.zip`. The action exports both `SUMMARY_PATH` and `ARTIFACT_PATH` so workflows can upload the single-file summary alongside the archive without duplicating business logic in each job.
...
- `resolver-smoke.yml` runs on pushes, pull requests, and manual dispatches. It executes the full resolver pipeline end-to-end using offline stubs: `resolver.ingestion.ifrc_go_stub` seeds `data/staging/<period>/raw`, `resolver.transform.normalize` canonicalizes every adapter-backed CSV discovered there, and `resolver.tools.load_and_derive` runs the `load-canonical`, `derive-deltas --allow-negatives 1`, and `export --format parquet` subcommands to produce header-safe snapshots. The job asserts that `facts_resolved.parquet` exists and uploads all Parquet files from the run as the `resolver-smoke-snapshots` artifact so PRs can prove the wiring without any network access.
...
Every resolver-facing workflow (`resolver-ci-fast`, `resolver-ci`, and `resolver-smoke`) now calls the reusable diagnostics collector so each job publishes a single-file summary plus one archive. You will see:

- `summary-<job>-<run_id>-<run_attempt>.md` – a standalone `SUMMARY.md` that mirrors the essentials in the job log (Git metadata, versions, sanitized environment, DuckDB table counts, staging/snapshot listings, pytest failures, and log tails).
- `diagnostics-<job>-<run_id>-<run_attempt>.zip` – the expanded pack containing the raw text snapshots (`versions.txt`, `env.txt`, `git.txt`, `staging.txt`, `snapshots.txt`, `duckdb.txt`, `pytest.txt`, and more) alongside the same `SUMMARY.md`.

In practice you will see `summary-fast-tests-<run>-<attempt>.md` and `diagnostics-fast-tests-<run>-<attempt>.zip` from the fast matrix plus `summary-pipeline-smoke-<run>-<attempt>.md` and `diagnostics-pipeline-smoke-<run>-<attempt>.zip` from the stubbed pipeline. Grab the summary when you want a quick triage pass, or download the archive if you need the underlying logs. The aggregate workflow still merges per-job summaries, but instead of uploading a second diagnostics pack it writes the consolidated markdown straight to the GitHub Actions job summary, keeping the “Artifacts” list predictable.
