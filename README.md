...
The IDMC (IDU) connector currently ships as an offline-only skeleton that loads bundled fixtures and normalizes displacement flow and stock metrics for testing.【F:resolver/ingestion/idmc/cli.py†L11-L84】【F:resolver/ingestion/idmc/normalize.py†L1-L125】 When `countries: []` (or the field is omitted) in `ingestion/config/idmc.yml`, the client targets the full ISO3 roster from `resolver/ingestion/static/iso3_master.csv` so empty runs are explicit rather than accidental.【F:resolver/ingestion/utils/country_utils.py†L118-L180】 You can override that scope by setting `IDMC_COUNTRIES="AFG,PAK"` (or pointing `IDMC_COUNTRIES_FILE` at a comma/whitespace separated file of ISO3 codes or country names); overrides cascade through the CLI, client, and diagnostics.【F:resolver/ingestion/utils/country_utils.py†L182-L209】【F:resolver/ingestion/idmc/cli.py†L221-L318】【F:resolver/ingestion/idmc/client.py†L470-L492】 Each run writes `diagnostics/ingestion/idmc/summary.md` with a dedicated Countries section and persists `diagnostics/ingestion/idmc/why_zero.json` that captures token presence, resolved country counts and samples, selected series, and date windows (even when you only enable `--debug`).【F:resolver/ingestion/idmc/cli.py†L60-L110】【F:resolver/diagnostics/templates/idmc_summary.md.j2†L1-L21】【F:resolver/ingestion/idmc/cli.py†L889-L936】

Choose how the connector reaches IDMC by supplying `--network-mode {live,cache_only,fixture}` (defaults to `live`) or by exporting `IDMC_NETWORK_MODE` before you run the CLI.【F:resolver/ingestion/idmc/cli.py†L173-L178】【F:resolver/ingestion/idmc/cli.py†L336-L357】 A typical staging run looks like `python -m resolver.ingestion.idmc.cli --write-candidates --network-mode live --debug`, while local cache rehearsals can rely on `IDMC_NETWORK_MODE=cache_only python -m resolver.ingestion.idmc.cli --write-outputs`. Diagnostics now record the effective `network_mode` alongside per-class HTTP status counters so that summaries and `why_zero.json` explicitly flag offline or cached executions.【F:resolver/ingestion/idmc/cli.py†L657-L713】【F:resolver/ingestion/idmc/cli.py†L912-L979】
