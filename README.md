...
All resolver workflows now execute critical commands through `scripts/ci/run_and_capture.sh`, a tiny wrapper that mirrors shell output to `.ci/diagnostics/<step>.log` and writes the corresponding status code to `.ci/exitcodes/<step>`. Fast tests, smoke runs, and nightly jobs therefore emit a uniform diagnostics bundle with:

- `pip-freeze.txt` captured via `scripts/ci/run_and_capture.sh pip-freeze "pip freeze > .ci/diagnostics/pip-freeze.txt"` so package mismatches are visible without rerunning the job.
- Canonical CSV summaries generated by `python scripts/ci/list_canonical.py --dir <canonical_dir> --out .ci/diagnostics/canonical-listing.txt`, which notes missing directories, file sizes, and header-safe row counts.
- DuckDB table statistics written by `python scripts/ci/db_counts.py --db data/resolver.duckdb --out .ci/diagnostics/duckdb-counts.txt`, gracefully skipping when the database is absent or DuckDB is not installed.
