...
- Every resolver workflow proactively uninstalls any cached wheel and runs `pip install -e .` so tests import the workspace copy of `resolver`. Immediately before the pytest steps we install the `db`/`test` extras (falling back to `pip install duckdb pytest` if the extras are unavailable) **with** `-c constraints-ci.txt` so the pinned connector dependencies (notably `dtmapi==0.1.5`) stay consistent across jobs. We also log a positive `import duckdb` probe so CI failures never stem from missing DuckDB wheels. After every pytest invocation—regardless of success—the shared composite action at `.github/actions/collect-diagnostics` emits a top-level `SUMMARY.md` (covering run metadata, sanitized environment details, pytest results, resolver logs, and DuckDB table counts) and stages those text snapshots under `dist/diagnostics-<job>-<run_id>-<attempt>/`. Workflows upload that directory so GitHub produces a single archive per job with the summary baked in.
...
- Fast DTM tests keep the connector active with soft timeouts and fake rescues so we still exercise normalization. Set `RESOLVER_SKIP_DTM=1` only when you truly need to bypass the connector; pair it with `DTM_FORCE_RUN=1` to override the skip in targeted scenarios. Header-only rescues now emit the three-column `CountryISO3,ReportingDate,idp_count` CSV and record a `zero_rows_reason` in diagnostics.
- **Codex users:** run `bash tools/codex_run_fast_tests.sh` to install the DuckDB/PyArrow/pandas test dependencies and execute the fast suite. The helper script sets safe defaults for offline execution without forcing IDMC into fixture mode, and `resolver/tests/conftest.py` leaves DTM enabled unless you explicitly set `RESOLVER_SKIP_DTM`.
- When running DB-marked tests offline, install the vendored DuckDB wheel via `./scripts/setup_test_deps_offline.sh`; the script installs `duckdb==1.1.0` from `vendor/wheels/` and emits a clear instruction if the wheel is missing.
- Environment switches such as `RESOLVER_SKIP_DTM`, `DTM_FORCE_RUN`, and `DTM_OFFLINE_SMOKE` are parsed with strict string handling—`"0"`, `"false"`, and `"no"` are treated as `False`, while `"1"`, `"true"`, and `"yes"` enable the flag—so setting `RESOLVER_SKIP_DTM="0"` keeps the connector active during fast tests.
...
The refactored DTM connector now focuses on API-first ingestion with a streamlined diagnostics footprint. See [`docs/ingestion/dtm.md`](docs/ingestion/dtm.md) for:

- Offline smoke instructions (`python -m resolver.ingestion.dtm_client --offline-smoke`).
...
- The official SDK expects ISO-3 selectors via the `Admin0Pcode` argument (not `CountryISO3`). The connector reads
  ...
  `CountryISO3` continue to pass while production requests send `Admin0Pcode` behind the scenes. The fast-test helper
  `_country_filter` mirrors the production contract (`Admin0Pcode` for ISO-3, otherwise `CountryName`), and diagnostics export
  `config_keys_found` as a dictionary (`{"countries": bool, "admin_levels": bool}`) to highlight whether selectors were
  provided explicitly.
...
- Configuration search order is `DTM_CONFIG_PATH` (absolute or repo-relative) →
  ...
  ISO3 codes iterated) in `diagnostics/ingestion/summary.md` under **Config used** alongside the final admin levels and
  countries mode (`explicit_config`, `discovered`, or `static_iso3_minimal`). Environment overrides (`DTM_COUNTRIES`,
  `DTM_ADMIN_LEVELS`) apply *after* the YAML is parsed so you can tweak selectors without touching the file. GitHub Actions
  workflows set `DTM_CONFIG_PATH=resolver/config/dtm.yml` so production runs keep the curated geography list without affecting
  fast-test defaults.
...
- The ingestion summary (`diagnostics/ingestion/summary.md`) now highlights the resolved DTM configuration, staging readiness
  (counts and sizes for both `resolver/staging` and the legacy `data/staging`), zero-row root causes (primary reason plus top
  drop reasons), selector effectiveness, and reachability diagnostics generated by `scripts/ci.probe_dtm_reachability` (DNS,
  TCP/TLS timings, HTTP `curl -I`, CA bundle, and current egress IPs). The new **Source sample: quick checks** block reads
  `diagnostics/ingestion/dtm/samples/admin0_head.csv` to render compact histograms for `CountryISO3` and `admin0Name` so you can
  spot alias issues without downloading artifacts. When a configured selector looks like an ISO3 code, the connector now queries
  the API with `CountryISO3=` first, falls back to `CountryName=` on “no match” responses, and records which path succeeded. A
  **DTM per-country results** table in `summary.md` lists the level, request parameter, page count, row count, and skip reason per
  selector so you can confirm the fallback and quickly spot unsupported countries. These sections appear ahead of the
  per-connector table and deep-dive details.
...
ACLED requests authenticate with a short-lived OAuth access token, so the client injects `Authorization: Bearer …` headers for every HTTP call and mirrors the raw status/url metadata to `diagnostics/acled/http_diag.json` for quick triage.【F:resolver/ingestion/acled_client.py†L1109-L1171】 The ACLED monthly fatalities CLI (`python -m resolver.cli.acled_to_duckdb`) keeps the `fields` parameter pipe-delimited (`event_date|iso3|…`) per the API contract and emits an "ACLED HTTP summary" block in both the GitHub step summary and `diagnostics/acled/duckdb_summary.md`, showing the requested window, page size, field mode, row counts, and a three-row preview.【F:resolver/ingestion/acled_client.py†L1223-L1286】【F:resolver/cli/acled_to_duckdb.py†L1-L191】 This mirrors the ingestion diagnostics so the DuckDB path surfaces the same request metadata even when the connector succeeds but normalization yields zero rows.

The IDMC (IDU) connector targets the live IDU API and now expects a concrete date window. Supply `--start`/`--end` on the CLI (or export `RESOLVER_START_ISO`/`RESOLVER_END_ISO`, falling back to `BACKFILL_*`) to wire a backfill span into the client; `--no-date-filter` still disables filtering when required. Diagnostics and the `why_zero.json` payload capture the window source, request counts, and staging row totals so empty runs explain whether the API returned zero features or the fetch never reached the network.【F:resolver/ingestion/idmc/cli.py†L300-L384】【F:resolver/ingestion/idmc/cli.py†L520-L611】【F:resolver/ingestion/idmc/cli.py†L900-L992】【F:resolver/ingestion/idmc/client.py†L516-L610】 When `countries: []` (or the field is omitted) in `resolver/config/idmc.yml`, the client targets the full ISO3 roster from `resolver/ingestion/static/iso3_master.csv`; override that scope via `IDMC_COUNTRIES`, `IDMC_COUNTRIES_FILE`, or `--only-countries` and the selection propagates through the client and diagnostics.【F:resolver/ingestion/utils/country_utils.py†L118-L209】【F:resolver/ingestion/idmc/cli.py†L223-L323】【F:resolver/ingestion/idmc/client.py†L520-L575】 Each run writes `diagnostics/ingestion/idmc/summary.md` with the resolved window, country sample, and staging counts alongside `diagnostics/ingestion/idmc/why_zero.json` for zero-row investigations.【F:resolver/ingestion/idmc/cli.py†L60-L122】【F:resolver/diagnostics/templates/idmc_summary.md.j2†L1-L21】【F:resolver/ingestion/idmc/cli.py†L900-L992】 The live client now emits only the canonical PostgREST filters (`displacement_start_date=gte.*`, `displacement_end_date=lte.*`, `iso3=in.(…)`, `limit`, `offset`, `select`) while chunk/window metadata stays in diagnostics; enable the mirrored HDX CSV rescue explicitly with `--allow-hdx-fallback` (or `IDMC_ALLOW_HDX_FALLBACK=1`) and it engages only when PostgREST never delivers a 2xx (timeouts, SSL failures, 4xx/5xx).【F:resolver/ingestion/idmc/cli.py†L212-L320】【F:resolver/ingestion/idmc/cli.py†L388-L471】【F:resolver/ingestion/idmc/client.py†L620-L910】【F:resolver/ingestion/idmc/client.py†L940-L1415】 Diagnostics capture every request (including sanitized query parameters) plus any fallback rows staged from HDX. When a fallback occurs the helper selects the configured `IDMC_HDX_RESOURCE_ID` (defaulting to the `idus_view_flat` CSV), downloads it once per run, verifies the payload exceeds 10 KB and still exposes the `iso3`/`figure` columns, and records the resource id/URL, byte size, and header sample for the CI summary to display.【F:resolver/ingestion/idmc/client.py†L531-L673】【F:scripts/ci/probe_hdx_reachability.py†L9-L210】【F:scripts/ci/summarize_connectors.py†L1617-L1653】

`resolver/config/idmc.yml` now ships with `allow_fallbacks: true` and `prefer_helix_last_180: true` defaults so workflows that rely on Helix or HDX rescue paths no longer need bespoke YAML overrides. The CLI still honours per-run overrides (for example `--allow-hdx-fallback` or `IDMC_ALLOW_HDX_FALLBACK=0`) but when a fallback succeeds it increments the runtime metrics counters (`rows_fetched`, `rows_normalized`, `rows_written`) and records staged row totals so `diagnostics/ingestion/idmc/summary.md` reports non-zero output even if the primary endpoint failed.【F:resolver/config/idmc.yml†L1-L23】【F:resolver/ingestion/idmc/client.py†L3866-L3920】【F:resolver/ingestion/idmc/cli.py†L1200-L1510】 The summary template renders through Jinja2 (`resolver/diagnostics/templates/idmc_summary.md.j2`) and falls back to the baked-in markdown when a syntax error slips into the template, ensuring diagnostics continue to publish a summary file for each run.【F:resolver/ingestion/idmc/cli.py†L40-L122】
Live runs also respect explicit HTTP tuning. The connector and the CI reachability probe advertise a configurable `User-Agent` (from `IDMC_USER_AGENT` or `RELIEFWEB_USER_AGENT`) and accept overrides for connect/read timeouts with `IDMC_HTTP_CONNECT_TIMEOUT_S`, `IDMC_CONNECT_TIMEOUT`, `IDMC_HTTP_READ_TIMEOUT_S`, or `IDMC_READ_TIMEOUT`; defaults remain 5 s/25 s and show up in diagnostics alongside timeout counters so soft failures never masquerade as green-but-empty runs.【F:resolver/ingestion/idmc/http.py†L1-L220】【F:resolver/ingestion/idmc/client.py†L560-L1112】【F:scripts/ci/probe_idmc_reprise.py†L1-L76】 The reachability probe appends an "IDMC Reachability" block (DNS/TCP/TLS/HTTP plus egress IP) to `diagnostics/ingestion/summary.md`, making workflow reviews highlight whether GitHub runners could connect at all.【F:scripts/ci/protocol_probe.py†L1-L205】【F:scripts/ci/probe_idmc_reprise.py†L26-L65】 CI enables `--allow-hdx-fallback`, `--debug`, and header writes for zero-row runs via `IDMC_WRITE_EMPTY=1` and `RESOLVER_EXPORT_ENABLE_FLOW=1` so backfills continue to land rows while fast tests keep the behaviour off by default for determinism.【F:.github/workflows/resolver-initial-backfill.yml†L180-L239】【F:resolver/ingestion/idmc/cli.py†L700-L912】 When `--write-outputs`, `RESOLVER_EXPORT_ENABLE_FLOW`, or `IDMC_WRITE_EMPTY=1` is supplied, the CLI writes `resolver/staging/idmc/flow.csv` with the canonical header even if normalization yields zero rows so exporters still detect the file.【F:resolver/ingestion/idmc/cli.py†L720-L804】【F:resolver/ingestion/idmc/staging.py†L9-L26】
The flow normalizer snaps every `as_of_date` to the month end and preserves the column as a `datetime64[ns]` dtype so downstream joins and comparisons stay within native pandas date semantics.【F:resolver/ingestion/idmc/normalize.py†L130-L211】

When `RESOLVER_OUTPUT_DIR` is exported (the CI workflows do this automatically) the CLI auto-enables `--write-outputs`, redirects the export directory to `${RESOLVER_OUTPUT_DIR}/idmc`, and writes `resolver/staging/idmc/flow.csv` with the `new_displacements` metric sourced from `idmc_idu`; optional Parquet and CSV facts land in the same folder when feature flags allow it.【F:resolver/ingestion/idmc/cli.py†L340-L438】【F:resolver/ingestion/idmc/export.py†L9-L47】【F:resolver/tools/export_config.yml†L70-L116】 Choose how the connector reaches IDMC by supplying `--network-mode {live,cache_only,fixture}` (defaults to `live`) or by exporting `IDMC_NETWORK_MODE` before you run the CLI; diagnostics record the effective mode together with per-class HTTP status counts (always the canonical `{"2xx","4xx","5xx"}` buckets), aggregated attempt timings, and any HDX fallbacks invoked during the run.【F:resolver/ingestion/idmc/cli.py†L178-L208】【F:resolver/ingestion/idmc/cli.py†L632-L740】【F:resolver/ingestion/idmc/client.py†L600-L736】 A typical staging run looks like `python -m resolver.ingestion.idmc.cli --network-mode live --start 2024-01-01 --end 2024-03-31 --series flow --debug`, while local cache rehearsals can rely on `IDMC_NETWORK_MODE=cache_only python -m resolver.ingestion.idmc.cli --start … --end …`.

### Verify an IDMC backfill

1. Run the connector with exports enabled (the example below writes staging CSVs, exports facts, and captures DuckDB stats):

   ```bash
   RESOLVER_EXPORT_ENABLE_IDMC=1 IDMC_WRITE_OUTPUTS=1 WRITE_TO_DUCKDB=1 \
   python -m resolver.ingestion.idmc.cli --network-mode live --start 2024-01-01 --end 2024-03-31
   ```

2. Inspect `diagnostics/ingestion/idmc/summary.md` to confirm the `Rows fetched/normalized/written` counters and the **DuckDB write** block report non-zero inserts for the requested window.
3. When DuckDB writes are enabled, run a quick coverage query against the configured database to verify row visibility:

   ```bash
   duckdb -c "SELECT COUNT(*) AS rows, MIN(as_of_date) AS first_month, MAX(as_of_date) AS last_month FROM facts_resolved WHERE source='idmc_idu';"
   ```

   The totals should match the `Rows inserted` line from `summary.md`.
