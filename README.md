...
- Every resolver workflow proactively uninstalls any cached wheel and runs `pip install -e .` so tests import the workspace copy of `resolver`. Immediately before the pytest steps we install the `db`/`test` extras (falling back to `pip install duckdb pytest` if the extras are unavailable) and log a positive `import duckdb` probe so CI failures never stem from missing DuckDB wheels. After every pytest invocation—regardless of success—we call `scripts/ci/package_diagnostics.sh`. That script sweeps the workspace for `pytest-junit.xml`, resolver logs, environment metadata, `pip freeze`, and any `*.duckdb` files under both the checkout and `/tmp/pytest-of-*`, then emits a `diagnostics_${JOB_NAME}_${DUCKDB_LABEL}.tar.gz` bundle alongside a mirrored `.ci/diagnostics/` tree. Each job uploads `diagnostics-<job>-<duckdb_label>` with those assets attached so triage always has a reproducible archive. The aggregate job simply downloads the tarballs, extracts them into `consolidated/`, and builds a run-level `SUMMARY.md` that inventories every captured file. Nightly legs reuse the same tooling when scheduled but add cache-disabled db-backend coverage on top.
