...
The resolver-ci workflow uploads **exactly one** consolidated artifact named `diagnostics-consolidated-<run_id>-<run_attempt>.zip`. It is assembled from the per-job bundles produced by `python -m resolver.tools.ci_diag_bundle` across the fast-tests workflow, the db-backend matrix, and the connector smokes, so the top-level `SUMMARY.md` captures every suite in one place. The “dream” summary records commit SHAs (merge and PR head), suite labels, DuckDB and Python versions, per-module SHA256 hashes and import paths, **auto-discovers every `*.duckdb` file created during the run**, records each table’s schema, unique indexes (with canonical column order), row counts, compact previews, declared-key PASS/FAIL verdicts, `ym` rollups, latest `as_of` probes, and a concise test failure digest with traceback snippets.
