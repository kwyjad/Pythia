...
@pytest.mark.asyncio
async def test_research_v2_includes_hazard_tail_pack(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    db_path = tmp_path / "research_tail.duckdb"
    monkeypatch.setenv("PYTHIA_DB_URL", f"duckdb:///{db_path}")
    monkeypatch.setenv("PYTHIA_HS_RESEARCH_WEB_SEARCH_ENABLED", "1")
    monkeypatch.setenv("PYTHIA_WEB_RESEARCH_ENABLED", "1")
    ...
    con.execute(
        ...
        [
            ...
            "tail query",
            "# tail pack",
            json.dumps([{"title": "tail", "url": "http://tail.com"}]),
            True,
            "tail structural",
            json.dumps(["TRIGGER | month_2-3 | UP | tail-signal"]),
        ],
    )
    con.close()
    ...
    assert "http://tail.com" in captured_prompts[0]
    assert "TRIGGER | month_2-3" in captured_prompts[0]
