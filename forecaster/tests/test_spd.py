...
@pytest.mark.db
def test_spd_member_names_are_deduplicated(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """Member rows should retain distinct names even when specs share the same label."""

    db_path = tmp_path / "spd_members_dupe.duckdb"
    monkeypatch.setenv("PYTHIA_DB_URL", f"duckdb:///{db_path}")
    monkeypatch.setenv("PYTHIA_SPD_V2_WRITE_BOTH", "1")
    ...
    question_row = {
        "question_id": "TEST_MEMBER_WRITE_DUPE",
        "hs_run_id": "",
        "scenario_ids_json": "[]",
        "iso3": "ETH",
        "hazard_code": "ACE",
        ...
        "window_start_date": None,
        "window_end_date": None,
        "wording": "Test member write dupe",
        "status": "active",
        "pythia_metadata_json": None,
    }
    ...
    specs = [
        ...
        ModelSpec(name="Gemini", provider="google", model_id="gemini-3-pro-preview", active=True),
    ]
    months = {f"2025-0{i}": [1.0 / len(SPD_CLASS_BINS)] * len(SPD_CLASS_BINS) for i in range(1, 7)}
    per_model_spds = [months, months]
    raw_calls = [
    ...
    asyncio.run(cli._run_spd_for_question("run-members-dupe", question_row))
    ...
    try:
        raw_counts = dict(
            con.execute(
                ...
                WHERE run_id=? AND question_id=? GROUP BY model_name
                """,
                ["run-members-dupe", "TEST_MEMBER_WRITE_DUPE"],
            ).fetchall()
        )
    finally:
        con.close()

    assert "Gemini (gemini-3-flash-preview)" in raw_counts
    assert "Gemini (gemini-3-pro-preview)" in raw_counts
    assert raw_counts["Gemini (gemini-3-flash-preview)"] == 6 * len(SPD_CLASS_BINS)
    assert raw_counts["Gemini (gemini-3-pro-preview)"] == 6 * len(SPD_CLASS_BINS)
